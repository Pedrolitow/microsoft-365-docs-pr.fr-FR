...
- Page **Antivirus** <a href="https://go.microsoft.com/fwlink/p/?linkid=2150463" target="_blank">dans Microsoft Endpoint Manager</a>. Voir [Gérer les détections de menaces Microsoft Endpoint Manager](#manage-threat-detections-in-microsoft-endpoint-manager) dans cet article.
...
2. Dans la page de navigation, sélectionnez **Périphériques** >  **actifs**. Vous verrez une liste des appareils actifs et des détails, tels que l’état de protection, l’état de protection antivirus et le nombre de menaces actives détectées.

3. Sélectionnez un appareil pour afficher plus de détails sur cet appareil et les actions disponibles. Un volant s’ouvre avec des recommandations et des actions disponibles, telles que la stratégie de mise à **jour,** **l’antivirus** de mise à **jour,** l’analyse **rapide,** l’analyse complète, etc.
...
La procédure suivante s’applique aux clients qui ont Microsoft 365 Business Premium. [Windows 10 appareils doivent être sécurisés](../setup/secure-win-10-pcs.md) et [inscrits Intune](/mem/intune/enrollment/windows-enrollment-methods).

> [!NOTE]
> La **carte** **Antivirus Microsoft Defender** la page Menaces actives sont déployées par phases, de sorte que vous n’avez peut-être pas un accès immédiat à ces cartes.
...
| Redémarrer l’appareil | Force un Windows 10 à redémarrer dans un délai de cinq minutes.<br><br>**IMPORTANT :** Le propriétaire ou l’utilisateur de l’appareil n’est pas averti automatiquement du redémarrage et risque de perdre du travail non annulé. |
...
3. Sous **Gérer**, sélectionnez **Antivirus**. Vous verrez plusieurs onglets, tels que **Résumé, Windows 10** points de terminaison **défectueux et Windows 10** **programmes malveillants détectés**.

4. Examinez les informations sous les onglets disponibles, puis prenez les mesures nécessaires.

Par exemple, supposons que les appareils sont répertoriés sous l Windows 10 **onglet programmes malveillants** détectés. Lorsque vous sélectionnez un appareil, certaines actions sont disponibles, telles que le redémarrage **,** l’analyse **rapide,** l’analyse **complète, la** synchronisation ou les **signatures de mise à jour**. Sélectionnez une action pour cet appareil.

Le tableau suivant décrit les actions que vous pouvez voir dans Microsoft Endpoint Manager.<br><br>

| Opération | Description |
|--|--|
| Redémarrer | Force un Windows 10 à redémarrer dans un délai de cinq minutes.<br><br>**IMPORTANT :** Le propriétaire ou l’utilisateur de l’appareil n’est pas averti automatiquement du redémarrage et risque de perdre du travail non annulé. |
...
| Synchronisation | Nécessite qu’un appareil s’y Intune (partie de Microsoft Endpoint Manager). Lorsque l’appareil est connecté, il reçoit les actions ou stratégies en attente attribuées à l’appareil. |
...
[Vue d’Microsoft Defender pour les PME](../../security/defender-business/mdb-overview.md) (Defender for Business est en déploiement Microsoft 365 Business Premium clients, à partir du 1er mars 2022)