...
Dans une heure ou après avoir quitté la page Microsoft 365 actuelle, ils sont invités à se reconnecter. Un jeton d’accès est valide pendant une heure. La chronologie dépend donc du temps restant sur ce jeton et de la sortie de sa page web actuelle.
...
1. Sélectionnez la boîte aux lettres de l’utilisateur dans la liste, puis, dans le *volet Détails* (à droite), sélectionnez **Gérer les paramètres des applications de messagerie** sous **Email applications**. **Désactivez** le curseur pour toutes les options ; **Mobile (Exchange ActiveSync)**, **Outlook sur le web**, **Bureau Outlook (MAPI),****services web Exchange**, **POP3** et **IMAP**.
