...
L’analyse de l’utilisation de Microsoft 365 se connecte à une API qui expose un modèle de données multidimensionnelles. Les API sont en préversion et sont accessibles à `https://reports.office.com/pbi/v1.0/\<tenantid\>` l’adresse (remplacez \<tenant id\> par le GUID de votre client). 
...
|Utilisation des boîtes aux lettres par les locataires  <br/> |Contient des données sur la boîte aux lettres de l’utilisateur, sur le nombre total de boîtes aux lettres et sur l’utilisation du stockage.  <br/> |Contient des données d’état de fin de mois pour une période de 12 mois, y compris le mois partiel actuel.  <br/> |
...
|État utilisateur  <br/> |Contient des métadonnées sur les utilisateurs, comme le nom d'affichage, les produits attribués, l'emplacement, le service, le titre et la société. Ces données sont des utilisateurs auxquels une licence a été attribuée au cours du dernier mois complet. Chaque utilisateur est représenté de manière unique par un ID d’utilisateur.  <br/> |Ces données concernent les utilisateurs qui disposaient d'une licence au cours du mois complet écoulé.  <br/> |
...
Ce tableau fournit des détails au niveau de l’utilisateur pour tous les utilisateurs auxquels une licence a été attribuée lors du dernier mois complet. Elle affiche des données provenant d'Azure Active Directory.
...
|IDType  <br/> |Le type d’ID est défini sur 1 si l’utilisateur est un utilisateur de Yammer qui se connecte à l’aide de son ID Yammer ou 0 s’il se connecte à Yammer à l’aide de son ID Microsoft 365.  <br/> La valeur est 1 pour indiquer que cet utilisateur se connecte à Yammer avec son ID Yammer et non avec son ID Microsoft 365  <br/> |
...
|YAM_State  <br/> |États de l’utilisateur dans le système Yammer, peut être actif, supprimé ou suspendu.  <br/> |
...
|EXO_EmailRead  <br/> |Nombre d’activités de lecture d’e-mails effectuées par l’utilisateur, il peut s’agir de plusieurs fois lors de la lecture d’un message électronique déjà lu ou d’un message électronique reçu précédemment.  <br/> |
...
Ce tableau fournit des données d’adoption mensuelles sur le mois en termes d’activation, d’activation, de renvoi et d’utilisateurs pour chaque produit de Microsoft 365. Les valeurs Microsoft 365 représentent une utilisation active dans l’un des produits.
...
Cette table fournit le nombre total mensuel d’activités et d’utilisateurs actifs pour diverses activités au sein des produits.
...
Ce tableau est constitué de données de synthèse pour tous les utilisateurs Exchange Online sous licence disposant d’une boîte aux lettres utilisateur. Elle contient un état de fin de mois englobant toutes les boîtes aux lettres utilisateur. Les données de cette table ne s'additionnent pas au fil des mois. Dans cette table, les données du mois précédent représentent l'état le plus récent.
  
|**Nom de la colonne**|**Description de la colonne**|
|:-----|:-----|
|TotalMailboxes  <br/> |Nombre de boîtes aux lettres utilisateur pour l’abonnement Microsoft 365.  <br/> |
|IssueWarningQuota  <br/> |Quota total pour l’émission d’un avertissement dans toutes les boîtes aux lettres des utilisateurs.  <br/> |
...
|MailboxesExceedSendReceiveQuota  <br/> |Nombre de boîtes aux lettres utilisateur ayant dépassé le quota d’envoi/réception.  <br/> |
...
Cette table se compose de données de synthèse mensuelles sur l’utilisation ou l’activité de sites SharePoint Online. Cela concerne uniquement les sites d’équipe et les sites de groupe. L’état de fin de mois des sites SharePoint Online est représenté dans cette colonne, par exemple, si un utilisateur a créé cinq documents et utilisé 10 Mo pour le stockage total, puis a supprimé certains fichiers et ajouté des fichiers de manière à ce que l’état de fin de mois de la fin du mois soit de sept au total. Cette table est masquée pour éviter le nombre d’agrégations en double et est utilisée comme source pour créer deux tableaux de référence.
...
|Dipland  <br/> |Total du stockage utilisé sur tous les comptes OneDrive à la fin de la période.  <br/> |
...
|SPO_FileAccessedActiveGroups  <br/> |Nombre de groupes SharePoint qui ont des activités accessibles dans le fichier.  <br/> |
...
Le tableau fournit des données sur le nombre d’activations d’abonnements Office dans les plans de service, par exemple, Microsoft 365 Apps for Enterprises, Visio, Project. Elle fournit également des données sur le nombre d'activations par appareil (Android/iOS/Mac/PC).
