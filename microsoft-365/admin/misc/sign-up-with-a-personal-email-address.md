---
title: Inscription à Office 365 avec une adresse de messagerie personnelle
f1.keywords:
- NOCSH
ms.author: pebaum
author: pebaum
manager: scotv
audience: Admin
ms.topic: article
ms.service: o365-administration
localization_priority: Normal
ms.collection:
- Adm_O365
- Adm_NonTOC
ms.custom: AdminSurgePortfolio
search.appverid:
- MET150
- MOE150
ms.assetid: 20816b8b-8b21-458f-81c1-320b8116597e
ROBOTS: NOINDEX
description: 'Découvrez comment s’inscrire à Office 365 avec votre adresse de messagerie personnelle, si vous ne souhaitez pas ajouter de domaine personnalisé. '
ms.openlocfilehash: b8e11d7c99ce385d8d5147245ac078b7d46c7caf
ms.sourcegitcommit: 628f195cbe3c00910f7350d8b09997a675dde989
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 10/21/2020
ms.locfileid: "48644878"
---
# <a name="signing-up-for-office-365-with-a-personal-email-address"></a>Inscription à Office 365 avec une adresse de messagerie personnelle

## <a name="when-you-skip-adding-a-custom-domain"></a>Lorsque vous ignorez l’ajout d’un domaine personnalisé

  
Lorsque vous vous inscrivez à Office 365 avec votre adresse de messagerie personnelle et que vous ignorez l’ajout d' [un domaine personnalisé](../get-help-with-domains/what-is-a-domain.md) pour le moment, vous limitez l’accès à certaines fonctionnalités. 
  
Si vous décidez d’ajouter un domaine personnalisé maintenant, vous pouvez accéder à toutes les fonctionnalités Premium ci-dessous :
  
|**Fonctionnalité**|**Office 365 avec courrier personnel**|**Office 365 avec un domaine personnalisé**|
|:-----|:-----|:-----|
|**OneDrive <sup>1, 2</sup>**| [OneDrive personnel ](https://onedrive.live.com/about/en-us/plans/)| [OneDrive Entreprise](https://onedrive.live.com/about/en-us/business/) |
|**Applications Office : Word, Excel, PowerPoint, OneNote, Outlook, Access (PC uniquement),**| Oui | Oui 
|**Applications métiers <sup>3</sup> : Microsoft Connections, facturation, listings, Bookings, MileIQ et gestionnaire de clients Outlook**| Non | Oui 
|**Accès au centre d’administration Microsoft 365**| Accès limité (configuration de facturation, de support et de domaine) | Oui
|**Ajouter des utilisateurs**| Non | Oui
|**Services Office 365 : Exchange Online, Skype entreprise, Microsoft Teams, SharePoint Online**| Non | Oui
|**Outils de sécurité et de conformité**| Non | Oui
   
 1. Vous devrez migrer vos [fichiers Onedrive personnels vers Onedrive entreprise](move-email-and-data-to-office-365-business-premium.md).
  
 2. Pour plus d’informations sur la façon dont vos données sont gérées, consultez la [déclaration de confidentialité](https://g.microsoftonline.com/0BX20en/138). L’utilisation de OneDrive est régie par le [contrat de services Microsoft](https://signup.live.com/signup?ru=https://login.live.com/oauth20_authorize.srf?lc%3d1033%26response_type%3dcode%26client_id%3d51483342-085c-4d86-bf88-cf50c7252078%26scope%3dopenid%2bprofile%2bemail%2boffline_access%26response_mode%3dform_post%26redirect_uri%3dhttps://login.microsoftonline.com/common/federation/oauth2%26state%3drQIIAXWRO2_TUACFc_NSUyGoEBKVEFIHJCSQk-vrR-JIHdLWSdPGaR426TVDZCeO7TjX17Ld5rGzd47EwgLqyFLED2DolBFVMIOYEBMjaXeWM5zvDEfnvEixebb8jOd4wSiaEiMZIsfwEgsZg0ciwwmcyCHIDgXIhQ83t64-PP_-ZvtGvnr04-uX8yedJcj1J-65lR9QcgkeO3EcROVCYTqd5ulo5A7uQOETACsAfgKwTGYsn9G6l8lI5EReYIWSCDmuJIhIKuYxkae6OiT6WIkx6nhNF0K80McN1eYVtR0rB56goKqD1YqgE5lde3wTVb071pOh3oWwSapuo3fkKKS-9vAcIwzx2JlgtT67ST44qZzFDroVGroL608yN6Ih6Qc0ipep9-AksPz6cJ_6vjWI87cxy4_dgRG71G-FNLDC2LWiXbOrtHuKZ7W0_nlgtvwOPZYJU-_7HsIYmWhPY2XSOz2WGh27PzY0ElRHbXgoOqo-N0Rq8KpNTbuzPy_CU1k7VJslRgtmc143vS6vwBrlup0SrYa-ViPzOdNTJHl_OGAachhVhnsz9WMqu56VUP86dX9dyneHO0FIR-7EWqXBr_Q9mCpvbGxuJbYTO4m_afAus36u-1qsZVblo7ffELCfgsR1pkCw8yps8Mh7SaJJaTGxXQU6MB7jakM_iyYHLQVJbRya07a9K5XZiyy4yGZ_ZxOfc_87-h81%26estsfed%3d1%26uaid%3ddd27a8b7188545dab714e7d8c6761b52%26lw%3d1%26fl%3deasi2%26mkt%3den-US&amp;mkt=EN-US&amp;uiflavor=web&amp;lw=1&amp;fl=easi2&amp;client_id=51483342-085c-4d86-bf88-cf50c7252078&amp;uaid=dd27a8b7188545dab714e7d8c6761b52&amp;lic=1). Le [contrat d’abonnement Microsoft Online](https://admin.microsoft.com/Commerce/Mosa.aspx?cc2=US&amp;cl=en&amp;cc=en-US&amp;gcc=False) régit tous les autres services inclus dans votre abonnement.
  
3. Certaines applications métiers ne sont pas disponibles dans toutes les régions.
  
## <a name="how-to-add-a-domain"></a>Procédure d’ajout d’un domaine

Dans le centre d’administration, accédez à **configuration**  >  <a href="https://go.microsoft.com/fwlink/p/?linkid=834818" target="_blank">Domains</a>  >  **Add Domain**.
  


