...
## <a name="baseline-identity-articles"></a>Articles sur l’identité de référence
...
- Je n’ai pas Active Directory et je ne veux pas l’installer. Azure AD Connecter peut être configuré pour [la synchronisation à partir de LDAP](/azure/active-directory/hybrid/plan-hybrid-identity-design-considerations-tools-comparison) (le partenaire peut être requis).
...
Il y a beaucoup d’autres personnes et des implications de processus. À mon avis, ce facteur est un domaine critique et sous-abordé. Peut-être plus dans un autre article.
...
3. Une fois tous les composants approuvés, assemblez ces éléments dans un ou plusieurs livrables (Azure) [Microsoft API Graph](/azure/active-directory/develop/microsoft-graph-intro) est votre meilleur ami ici. Pas impossible, mais beaucoup plus complexe pour concevoir une solution couvrant [plusieurs locataires](/azure/active-directory/develop/single-and-multi-tenant-apps).
