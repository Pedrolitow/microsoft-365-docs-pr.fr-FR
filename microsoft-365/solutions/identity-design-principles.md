...
- **Bonnes pratiques :** reconnaître que les meilleures pratiques changent à la vitesse de la lumière. Si vous avez examiné la Azure AD il y a plus de trois mois, vous êtes probablement hors de la date. Tout ce qui est ici est sujet à modification après la publication. La « meilleure » option d’aujourd’hui peut ne pas être la même dans six mois.
...
- Il n’existe pas de concept de sous/super client. Pour une raison quelconque, cette répétition se répète toujours. Cela [s’applique Azure AD client B2C](/azure/active-directory-b2c/) également. J’entends trop souvent « Mon environnement B2C se trouve dans mon client XYZ » ou « Comment déplacer mon client Azure vers mon client Office 365 client ? »
...
[Azure AD Connecter](/azure/active-directory/hybrid/whatis-azure-ad-connect) vs. [Microsoft Identity Manager](/microsoft-identity-manager/microsoft-identity-manager-2016) (MIM) et quelque chose d’autre (tiers ou personnalisé) ? Économisez-vous beaucoup de difficultés maintenant et à l’avenir et Azure AD Connecter. Il existe toutes sortes d’intelligences dans cet outil pour répondre aux configurations client et aux innovations en cours.
...
- Je n’ai pas Active Directory et je ne souhaite pas l’installer. Azure AD Connecter pouvez configurer la synchronisation à partir [de LDAP](/azure/active-directory/hybrid/plan-hybrid-identity-design-considerations-tools-comparison) (le partenaire peut être requis).
...
Le moteur de stratégie de Azure AD est implémenté à l’aide de [stratégies d’accès conditionnel.](/azure/active-directory/conditional-access/overview) Ce système dépend des informations de divers autres systèmes de détection des menaces pour prendre des décisions dynamiques. Un affichage simple ressemble à l’illustration suivante :
...
- Si vous téléchargez un volume de données anormalement élevé, tout ce que vous téléchargez sera chiffré et restreint.
...
- [Microsoft Defender pour les applications cloud](/cloud-app-security/) (Defender pour les applications cloud)
...
Bien entendu, en plus des Azure AD, différents services et applications ont leurs propres modèles d’autorisation spécifiques. Certains d’entre eux sont abordés plus loin dans la section délégation.

### <a name="audit"></a>Audit

Azure AD fonctionnalités [d’audit](/azure/active-directory/reports-monitoring/) et de rapports détaillées. Toutefois, il ne s’agit généralement pas de la seule source d’informations nécessaire pour prendre des décisions de sécurité. Pour plus d’informations à ce sujet, voir la section délégation.
...
Dans l’ensemble, il est plus difficile de tracer une ligne claire entre Office 365 services dans les clouds Microsoft. Je considère qu’il s’agit d’un avantage pour les clients, car ils peuvent bénéficier d’une innovation totale dans tout ce que nous faisons, même s’ils utilisent un seul composant. Assez cool et a des implications très importantes pour de nombreux clients.
...
### <a name="azure-ad-and-microsoft-365-admin-centers"></a>Azure AD et Microsoft 365 centres d’administration
...
- **Administrateur général**: ce rôle « [](../enterprise/protect-your-global-administrator-accounts.md) tout puissant » doit être hautement protégé, comme vous le feriez dans d’autres systèmes. Les recommandations classiques sont les suivantes : aucune affectation permanente et utilisation de Azure AD Privileged Identity Management (PIM), une authentification forte, etc. Il est intéressant de noter que ce rôle ne vous donne pas accès à tous les accès par défaut. En règle générale, je vois une confusion concernant l’accès à la conformité et l’accès Azure, abordée plus loin. Toutefois, ce rôle peut toujours attribuer l’accès à d’autres services dans le client.
- **Administrateurs de service spécifiques**: certains services (Exchange, SharePoint, Power BI, etc.) utilisent des rôles d’administration de haut niveau de Azure AD. Cela n’est pas cohérent dans tous les services et d’autres rôles spécifiques au service sont abordés ultérieurement.
...
**Remarque**: le Centre d'administration Microsoft 365 a une interface plus conviviale, mais possède un sous-ensemble de fonctionnalités par rapport à l’expérience Azure AD’administration. Les deux portails utilisent les mêmes rôles Azure AD, de sorte que les modifications se produisent au même endroit. Conseil : si vous souhaitez une interface utilisateur d’administration axée sur la gestion des identités sans tout l’encombrement Azure, utilisez <https://aad.portal.azure.com> .
...
La possibilité de créer des [rôles personnalisés](/azure/active-directory/users-groups-roles/roles-custom-overview) est une question courante. Ce nombre est limité dans Azure AD jour (voir ci-dessous), mais augmente au fil du temps. Je les trouve applicables aux fonctions dans Azure AD et ne peuvent pas s’étendre « vers le bas » du modèle hiérarchique (abordé ci-dessus). Chaque fois que j’ai affaire à « personnalisé », j’ai tendance à revenir à mon principal de « simple est préférable ».
...
Aujourd’hui, tous ces rôles nécessitent une appartenance directe (ou une affectation dynamique si vous utilisez [Azure AD PIM).](/azure/active-directory/privileged-identity-management/) Cela signifie que les clients doivent les gérer directement Azure AD et qu’ils ne peuvent pas être basés sur l’appartenance à un groupe de sécurité. Je ne suis pas un fan de la création de scripts pour les gérer, car il faudrait qu’ils s’exécutent avec des droits élevés. Je recommande généralement l’intégration d’API à des systèmes de processus tels que ServiceNow ou à l’aide d’outils de gouvernance partenaires tels que Saviynt. Des travaux techniques sont en cours pour résoudre ce problème au fil du temps.

[J’ai mentionné Azure AD PIM](/azure/active-directory/privileged-identity-management/) à plusieurs reprises. Il existe une solution Microsoft Identity Manager (MIM) [Privileged Access Management](/microsoft-identity-manager/pam/privileged-identity-management-for-active-directory-domain-services) (PAM) correspondante pour les contrôles locaux. Vous souhaitez peut-être également examiner les stations de travail [à](/windows-server/identity/securing-privileged-access/privileged-access-workstations) accès privilégié et [la gouvernance Azure AD identité.](/azure/active-directory/governance/identity-governance-overview) Il existe également différents outils tiers, qui peuvent activer l’élévation de rôle dynamique, juste-à-temps et juste-à-temps. Cela fait généralement partie d’une discussion plus large sur la sécurisation d’un environnement.
...
[Les autorisations dans](../security/office-365-security/permissions-in-the-security-and-compliance-center.md) le Centre Office 365 sécurité & conformité sont une collection de « groupes de rôles », qui sont distincts des rôles Azure AD rôles. Cela peut prêter à confusion, car certains de ces groupes de rôles ont le même nom que les rôles Azure AD (par exemple, lecteur sécurité), mais ils peuvent avoir des appartenances différentes. Je préfère utiliser les rôles Azure AD de gestion. Chaque groupe de rôles se compose d’un ou plusieurs « rôles » (voir ce que je veux dire sur la réutilisation du même mot ?) et ont des membres de Azure AD, qui sont des objets à messagerie. En outre, vous pouvez créer un groupe de rôles avec le même nom qu’un rôle, qui peut contenir ou non ce rôle (évitez cette confusion).
...
Ces groupes de rôles nécessitent également une appartenance directe et ne peuvent pas contenir Azure AD groupes. Malheureusement, aujourd’hui, ces groupes de rôles ne sont pas pris en charge par Azure AD PIM. Comme Azure AD rôles, j’ai tendance à recommander leur gestion par le biais d’API ou d’un produit de gouvernance partenaire tel que Saviynt, ou d’autres.
...
- **Plateforme Power -** (/power-platform/admin/admin-documentation)
  ...
  - **Power BI** - (/power-bi/service-admin-governance)
    ...
    Remarque : la sécurité et la délégation de plateforme de données (qui Power BI est un composant) est un domaine complexe.
...
- **Microsoft Defender pour les applications cloud** - (/cloud-app-security/manage-admins)
...
- Systèmes UEBA/menaces abordés ci-dessus (par exemple, Azure AD Identity Protection, Microsoft Defender pour les applications cloud, Microsoft Defender pour le point de terminaison, et ainsi de suite)
