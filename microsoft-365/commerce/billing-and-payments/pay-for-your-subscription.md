...
description: Découvrez les options de paiement disponibles pour le paiement de votre abonnement Office 365 pour les entreprises.
ms.custom:
- okr_SMB
...
Lorsque vous configurez votre abonnement, vous pouvez choisir de payer par carte bancaire ou par compte bancaire, ou de payer par facture à l’aide d’un chèque ou d’un transfert électronique de fonds (TEF). Quelle que soit la méthode que vous choisissez, nous continuerons d'utiliser ce mode de paiement pour les périodes de facturation suivantes, sauf si vous [modifiez votre mode de paiement](change-payment-method.md).

> [!NOTE]
> Le paiement par compte bancaire n’est pas disponible dans certains pays ou certaines régions.

## <a name="pay-by-credit-card-or-bank-account"></a>Payer par carte bancaire ou compte bancaire
...
Si vous payez par carte bancaire et que cette dernière est refusée, vous recevez un courrier électronique vous informant que nous n'avons pas pu procéder au paiement. Cette situation peut se produire pour plusieurs raisons, notamment à cause de l'expiration de la carte bancaire. Nous continuerons d'essayer d'effectuer la transaction pendant quelques jours. Si ces tentatives échouent, vous recevrez des alertes par courrier électronique concernant ces échecs. Vous pouvez utiliser la fonction **solde de règlement** à tout moment pour modifier les informations de votre carte de crédit et renvoyer immédiatement le paiement.
...
3. Si vous êtes le propriétaire de la carte de crédit, vous pouvez modifier la date d’expiration de la carte, choisir une autre carte ou [Ajouter une nouvelle carte](add-update-or-remove-credit-card-or-bank-account.md).

    > [!NOTE]
    > Lorsque vous utilisez la fonction **solde de règlement** , vous devrez payer le solde impayé total de votre compte, qui peut être supérieur au montant dû indiqué ailleurs.

4. Si vous n’êtes pas le propriétaire de la carte de crédit, vous pouvez choisir une autre carte de crédit ou [Ajouter une nouvelle carte](add-update-or-remove-credit-card-or-bank-account.md).

5. Une fois que vous avez mis à jour les informations de la carte de crédit ou modifié la carte de crédit utilisée pour payer l’abonnement, sélectionnez **Enregistrer**.
...
5. Une fois que vous avez mis à jour les informations de la carte de crédit ou modifié la carte de crédit utilisée pour payer l’abonnement, sélectionnez **Envoyer**. Un message de confirmation s’affiche indiquant que votre mode de paiement a été mis à jour et que tous les frais restants sont réglés. Merci.

::: moniker-end

## <a name="pay-by-invoice-check-or-eft"></a>Payer par facture (chèque ou TEF)
...
> [!NOTE]
> Les clients qui vivent au Brésil peuvent payer un abonnement avec un Bancario Boleto. Si vous avez sélectionné cette option, le billet est envoyé à l’e-mail fourni lors de l’achat de l’abonnement dans les 10 jours ouvrés suivant la date de la commande. La date d’échéance est 30 jours après la date de la commande. Si vous ne recevez pas votre Boleto par courrier électronique, vérifiez votre dossier de courrier indésirable ou contactez le support technique. <br><br>Si vous le souhaitez, vous pouvez payer par virement électronique entre les comptes. Le numéro d’Agence et de compte se trouve en bas de votre facture. Vous devez entrer votre numéro de facture dans le champ identification de transfert.
...
> [!NOTE]
> Si vous optez pour le paiement automatique par carte bancaire, n'oubliez pas de régler le solde restant. Consultez votre relevé de facturation pour obtenir des instructions et voir les options de paiement qui s'offrent à vous. Pour plus d’informations, reportez-vous à [la rubrique comprendre votre facture](understand-your-invoice2.md).
...
## <a name="receive-a-copy-of-your-billing-statement-in-email"></a>Envoyer une copie de votre relevé de facturation par courrier électronique
...
### <a name="to-receive-your-billing-statement-in-email"></a>Pour recevoir votre relevé de facturation par courrier électronique

1. Dans le centre d’administration, accédez à la page <a href="https://go.microsoft.com/fwlink/p/?linkid=853212" target="_blank">notifications de facturation</a> de la **facturation** \> .

    Si vous utilisez Office 365 Germany, accédez à cette page <a href="https://go.microsoft.com/fwlink/p/?linkid=853213" target="_blank">notifications de facturation</a> .
...
Si les raisons qui vous ont poussé à choisir un mode de paiement par facture, carte bancaire ou compte bancaire ne sont plus d'actualité (par exemple, si vous avez modifié votre abonnement de sorte que le montant à payer est inférieur au seuil de paiement par facture), vous pouvez changer de mode de paiement. Pour plus d’informations, consultez [la rubrique modifier votre mode de paiement](change-payment-method.md).
