...
|`IsGuestUser`|`boolean`|Indique si l’utilisateur qui s’est inscrit est un invité dans le client|
|`AlternateSignInName`|`string`|Nom d’utilisateur principal (UPN) local de l’utilisateur qui se Azure AD|
|`LastPasswordChangeTimestamp`|`datetime`|Date et heure de la dernière fois où l’utilisateur qui s’est inscrit a modifié son mot de passe|
