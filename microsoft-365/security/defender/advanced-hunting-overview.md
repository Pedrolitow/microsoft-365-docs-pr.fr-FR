...
Pour utiliser le hunting avancé ou d'autres [fonctionnalités de Microsoft 365 Defender,](microsoft-365-defender.md) vous avez besoin d'un rôle approprié dans Azure Active Directory. [En savoir plus sur les rôles et autorisations requis pour le chasse avancée](custom-roles.md).

En outre, votre accès aux données de point de terminaison est déterminé par les paramètres de contrôle d'accès basé sur un rôle (RBAC) dans Microsoft Defender for Endpoint. [En savoir plus sur la gestion de l'accès à Microsoft 365 Defender](m365d-permissions.md).
