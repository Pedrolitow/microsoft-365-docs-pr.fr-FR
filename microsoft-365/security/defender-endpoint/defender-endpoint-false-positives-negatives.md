...
2. **Sélectionnez la file d’attente des alertes**, puis une alerte.
...
Lorsque vous avez terminé de passer en revue et d’annuler les actions qui ont été prises à la suite de faux positifs, examinez ou définissez [les exclusions](#part-3-review-or-define-exclusions).
...
5. Choisissez **Révision + Enregistrer**, puis **Enregistrer**.
...
Pour spécifier des entités en tant qu’exclusions pour Microsoft Defender pour le point de terminaison, créez des indicateurs « autoriser » pour ces entités. Ces indicateurs « autoriser » dans Microsoft Defender pour point de terminaison s’appliquent à la [protection](microsoft-defender-antivirus-in-windows-10.md) [nouvelle génération](overview-endpoint-detection-response.md), aux protection évolutive des points de terminaison et aux examens automatisés & [correction](/microsoft-365/security/defender-endpoint/automated-investigations).
...
Lorsque vous [créez un indicateur «](/microsoft-365/security/defender-endpoint/indicator-file) autoriser » pour un fichier, tel qu’un exécutable, cela empêche le blocage des fichiers que votre organisation utilise. Les fichiers peuvent inclure des fichiers exécutables portables (PE), tels que des `.exe` fichiers `.dll` .
...
Lorsque vous [créez](/microsoft-365/security/defender-endpoint/indicator-ip-domain) un indicateur « autoriser » pour une adresse IP, une URL ou un domaine, cela empêche le blocage des sites ou des adresses IP que votre organisation utilise.
...
Les indicateurs réseau personnalisés sont allumés dans le [Microsoft 365 Defender](/microsoft-365/security/defender/microsoft-365-defender). Pour plus d’informations, voir [Fonctionnalités avancées](/microsoft-365/security/defender-endpoint/advanced-features).

#### <a name="indicators-for-application-certificates"></a>Indicateurs pour les certificats d’application

Lorsque vous [créez un](/microsoft-365/security/defender-endpoint/indicator-certificates) indicateur « autoriser » pour un certificat d’application, cela empêche le blocage des applications, telles que les applications développées en interne, que votre organisation utilise. `.CER` ou les `.PEM` extensions de fichier sont pris en charge.
...
Pour vérifier les mises à jour relatives à votre soumission, connectez-vous [au site Renseignement de sécurité Microsoft soumission](https://www.microsoft.com/wdsi/filesubmission).
...
3. Sous **Gérer**, choisissez **Propriétés**, puis, en plus des **paramètres de configuration**, sélectionnez **Modifier**.

4. Sous **l’onglet Paramètres de configuration**, faites défiler vers le bas et **développez Antivirus Microsoft Defender**.

5. Définissez **Détecter les applications potentiellement indésirables** sur **Audit**. (Vous pouvez la désactiver, mais en utilisant le mode audit, vous pourrez voir les détections.)

6. Choisissez **Révision + Enregistrer**, puis **Enregistrer**.
