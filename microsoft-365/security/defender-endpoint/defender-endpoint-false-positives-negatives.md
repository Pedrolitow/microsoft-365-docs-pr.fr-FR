...
Les alertes peuvent être classées en tant que faux positifs ou vrais positifs dans le Centre de sécurité Microsoft Defender. La classification des alertes permet de former Microsoft Defender pour le point de terminaison afin que, au fil du temps, vous verrez plus d'alertes vraies et moins de fausses alertes.
...
2. Entrez la commande suivante, puis appuyez sur **Entrée**:
    ...
    > [!NOTE]
    > Dans certains scénarios, **threatName** peut apparaître comme : `EUS:Win32/
CustomEnterpriseBlock!cl` . Defender for Endpoint restaure tous les fichiers bloqués personnalisés mis en quarantaine sur cet appareil au cours des 30 derniers jours.
...
En règle générale, il n'est pas nécessaire de définir des exclusions pour l'Antivirus Microsoft Defender. Veillez à définir les exclusions avec parcimonie et à n'inclure que les fichiers, dossiers, processus et fichiers ouverts par processus qui entraînent des faux positifs. En outre, veillez à examiner régulièrement vos exclusions définies. Nous vous recommandons [d'utiliser Microsoft Endpoint Manager pour](/mem/endpoint-manager-overview) définir ou modifier vos exclusions antivirus . toutefois, vous pouvez utiliser d'autres méthodes, telles que la stratégie de [groupe](/azure/active-directory-domain-services/manage-group-policy) (voir [Gérer Microsoft Defender pour le point de terminaison).](manage-atp-post-migration.md)

> [!TIP]
> Vous avez besoin d'aide sur les exclusions antivirus ? Voir Configurer et valider les exclusions pour les analyses de [l'Antivirus Microsoft Defender.](/windows/security/threat-protection/microsoft-defender-antivirus/configure-exclusions-microsoft-defender-antivirus)
...
Les indicateurs réseau personnalisés sont allumés dans le Centre de sécurité Microsoft Defender (voir [Fonctionnalités avancées)](/microsoft-365/security/defender-endpoint/advanced-features)   
...
Nous vous recommandons [d'utiliser Microsoft Endpoint Manager pour](/mem/endpoint-manager-overview) modifier ou définir vos paramètres de protection cloud . toutefois, vous pouvez utiliser d'autres méthodes, telles que la stratégie de [groupe](/azure/active-directory-domain-services/manage-group-policy) (voir [Gérer Microsoft Defender pour le point de terminaison).](manage-atp-post-migration.md)
...
Nous vous recommandons [d'utiliser Microsoft Endpoint Manager pour](/mem/endpoint-manager-overview) modifier ou définir les paramètres de protection PUA . toutefois, vous pouvez utiliser d'autres méthodes, telles que la stratégie de [groupe](/azure/active-directory-domain-services/manage-group-policy) (voir [Gérer Microsoft Defender pour le point de terminaison).](manage-atp-post-migration.md)
