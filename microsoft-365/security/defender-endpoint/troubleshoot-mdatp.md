...
---
# <a name="troubleshoot-service-issues"></a>Résoudre les problèmes de service
...
Si certains éléments ou données d’interface utilisateur sont manquants dans le Centre de sécurité Microsoft Defender, il est possible que les paramètres proxy le bloquent.

Assurez-vous `*.securitycenter.windows.com` qu’elle est incluse dans la liste d’accès proxy.
...
Consultez la rubrique [Examiner les événements](event-error-codes.md) et les erreurs à l’aide de l’Observateur d’événements pour obtenir la liste des ID d’événement signalés par le service Microsoft Defender for Endpoint. Cette rubrique contient également les étapes de résolution des erreurs d’événement.
...
Lorsque vous utilisez le Centre de sécurité Azure pour surveiller les serveurs, un client Microsoft Defender pour point de terminaison est créé automatiquement. Les données de Microsoft Defender pour point de terminaison sont stockées en Europe par défaut.
