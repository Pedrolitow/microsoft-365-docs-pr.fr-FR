...
- Lorsque Antivirus Microsoft Defender est en mode actif, PEPT en mode blocage ne fournit pas d’analyse supplémentaire, mais permet à Defender for Endpoint d’prendre des mesures automatiques sur les détections de PEPT comportement après violation.
...
|Invite de commandes     | 1. Sélectionnez le menu Démarrer, commencez à taper, puis ouvrez Windows invite de commandes `Command Prompt` dans les résultats. <p>2. `sc query windefend` Tapez . <p>3. Dans la liste des résultats, dans la ligne **STATE,** confirmez que le service est en cours d’exécution.         |
...
Si Windows Server 2016 a Antivirus Microsoft Defender en mode actif et que le point de terminaison est intégré à Defender pour le point de terminaison, PEPT en mode bloc est techniquement pris en charge. Toutefois, PEPT en mode bloc est conçu pour être une protection supplémentaire lorsque Antivirus Microsoft Defender n’est pas la solution antivirus principale sur un point de terminaison. Dans ce cas, Antivirus Microsoft Defender s’exécute en mode passif. 
