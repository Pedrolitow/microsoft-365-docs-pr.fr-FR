...
3. Exécutez `sudo mdatp diagnostic create` cette session pour enregistrer les journaux microsoft Defender pour les points de terminaison. Les fichiers sont stockés dans une archive .zip. Cette commande imprime également le chemin d’accès du fichier à la sauvegarde une fois l’opération réussie.
...
Il existe plusieurs façons de désinstaller Microsoft Defender pour Endpoint pour Mac. Notez que bien que la désinstallation gérée de manière centralisée soit disponible sur JAMF, elle n’est pas encore disponible pour Microsoft Intune.

### <a name="interactive-uninstallation"></a>Désinstallation interactive

- Ouvrez **Finder > Applications.** Cliquez avec le bouton **droit sur Microsoft Defender ATP > déplacer vers la corbeille.**
...
- Exécutez les commandes suivantes pour activer lacompletion automatique pour Microsoft Defender pour Endpoint pour Mac et redémarrer la session Terminal :
