...
- Azure AD connexion est configurée pour synchroniser les objets de l’ordinateur. Par défaut, les unités d’organisation d’ordinateur sont dans Azure AD étendue de synchronisation de connexion. Si les objets ordinateur appartiennent à des unités d’organisation spécifiques, configurez les unités d’organisation pour qu’elles se synchronisent dans Azure AD Connecter. Pour en savoir plus sur la synchronisation des objets ordinateur à l’aide de Azure AD Connecter, consultez [filtrage basé sur les unités de l’organisation](/azure/active-directory/hybrid/how-to-connect-sync-configure-filtering#organizational-unitbased-filtering).

> [!IMPORTANT]
> Azure AD connect ne synchronise pas les objets ordinateur Windows Server 2012 R2. Si vous devez les inscrire auprès de Azure AD for Security Management for Microsoft Defender pour point de terminaison, vous devez personnaliser Azure AD connecter la règle de synchronisation pour inclure ces objets d’ordinateur dans l’étendue de synchronisation. Consultez [les instructions d’application de la règle de jointure d’ordinateur dans Azure Active Directory Connecter]().
...
|`36`|Erreur de l’API LDAP|L’appareil a été correctement intégré à Microsoft Defender pour point de terminaison. Toutefois, une erreur s’est produite dans le flux de gestion de la configuration de la sécurité en raison d’une configuration incorrecte dans AAD Connecter. Pour identifier ce qui empêche l’appareil de s’inscrire à AAD, envisagez d’exécuter [l’outil de résolution des problèmes d’inscription](/samples/azure-samples/dsregtool/dsregtool) d’appareil. Pour Windows Server 2012 R2, exécutez les [instructions de dépannage dédiées](/azure/active-directory/devices/troubleshoot-hybrid-join-windows-legacy).  |
|`37`|Problème de synchronisation locale|L’appareil a été correctement intégré à Microsoft Defender pour point de terminaison. Toutefois, une erreur s’est produite dans le flux de gestion de la configuration de la sécurité en raison d’une configuration incorrecte dans AAD Connecter. Pour identifier ce qui empêche l’appareil de s’inscrire à AAD, envisagez d’exécuter [l’outil de résolution des problèmes d’inscription](/samples/azure-samples/dsregtool/dsregtool) d’appareil. Pour Windows Server 2012 R2, exécutez les [instructions de dépannage dédiées](/azure/active-directory/devices/troubleshoot-hybrid-join-windows-legacy). |
...
À partir des informations contenues dans le message, il est possible dans la plupart des cas de comprendre quelle erreur a été rencontrée, quelle API Win32 a retourné l’erreur (le cas échéant), quelle URL (le cas échéant) a été utilisée et quelle AAD erreur d’API Runtime a été rencontrée.
...
Pour la gestion de la sécurité pour les Microsoft Defender pour point de terminaison sur Windows Server 2012 ordinateurs joints à un domaine R2, une mise à jour de la règle de synchronisation Azure AD Connecter « In from AD-Computer Join » est nécessaire. Pour ce faire, vous pouvez cloner et modifier la règle, ce qui désactivera la règle « In from AD - Computer Join » d’origine. Azure AD Connecter offre par défaut cette expérience pour apporter des modifications aux règles intégrées.
