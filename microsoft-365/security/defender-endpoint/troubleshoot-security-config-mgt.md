...
Pour plus d’informations sur la gestion de la sécurité pour Microsoft Defender pour le point de [terminaison, voir Gérer Microsoft Defender pour le point](/mem/intune/protect/mde-security-integration) de terminaison sur les appareils Microsoft Endpoint Manager.

Pour obtenir des instructions sur la gestion de la sécurité pour Microsoft Defender pour l’intégration de point de [terminaison, voir Microsoft Defender pour endpoint Security Configuration Management](security-config-management.md)
...
Dans **l’inventaire des appareils** \> **de** points  de terminaison, la colonne Géré par a été ajoutée au filtre par canal de gestion (par exemple, MEM).
...
Pour voir la liste de tous les appareils qui ont échoué au processus de gestion de la sécurité de Microsoft Defender pour l’intégration de point de terminaison, filtrez le tableau par **MDE-Error**.
...
## <a name="run-microsoft-defender-for-endpoint-client-analyzer-on-windows"></a>Exécutez Microsoft Defender pour Endpoint Client Analyzer sur Windows
...
Si vous n’avez pas pu identifier l’appareil intégré dans AAD ou MEM et que vous n’avez pas reçu d’erreur lors de l’inscription, `Computer\\HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\SenseCM\\EnrollmentStatus` la vérification de la clé de Registre peut fournir des informations de dépannage supplémentaires.

:::image type="content" source="images/enrollment-status.png" alt-text="Page affichant l’état d’inscription" lightbox="images/enrollment-status.png":::
...
:::image type="content" source="images/event-properties.png" alt-text="Page des propriétés de l’événement" lightbox="images/event-properties.png":::

À partir des informations du message, il est possible dans la plupart des cas de comprendre quelle erreur s’est produite, quelle API Win32 a renvoyée l’erreur (le cas échéant), quelle URL (le cas échéant) a été utilisée et quelle erreur d’API runtime AAD été rencontrée.
...
2. Avec la **règle Entrée à partir d’AD – Joint** de l’ordinateur en surbrillant, sélectionnez **Modifier**. Dans la **boîte de dialogue Modifier la confirmation des** règles réservées, sélectionnez **Oui**.

   :::image type="content" source="images/8854440d6180a5580efda24110551c68.png" alt-text="Page de confirmation de la modification des règles réservées" lightbox="images/8854440d6180a5580efda24110551c68.png":::
...
4. **Sélectionnez Trois fois** suivants. Cela permet d’accéder à la section « Transformations » de la règle. Ne modifiez pas les sections « Filtre d’portée » et « Règles de jointage » de la règle. La section « Transformations » doit maintenant être affichée.
   ...
   :::image type="content" source="images/296f2c2a705e41233631c3784373bc23.png" alt-text="La règle de synchronisation entrante" lightbox="images/296f2c2a705e41233631c3784373bc23.png":::
