...
| ``15``                        |Insématisation du client                        |L’appareil a été correctement intégré à Microsoft Defender pour le point de terminaison. Toutefois, une erreur s’est produite dans le flux de gestion de la configuration de la sécurité, car votre ID de client Microsoft Defender pour point de terminaison ne correspond pas à votre ID de client Azure Active Directory.Assurez-vous que l’ID de client Azure Active Directory de votre client Defender for Endpoint correspond à l’ID de client dans l’entrée SCP de votre domaine. Pour plus d’informations, résolution des problèmes d’intégration liés à la gestion de la [sécurité pour Microsoft Defender pour le point de terminaison.](troubleshoot-security-config-mgt.md)|
