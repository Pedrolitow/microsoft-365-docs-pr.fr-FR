...
4. Copiez **ProcMon.exe**  vers le client Windows ou le serveur Windows que vous dépannagez.  
...
Vous pouvez utiliser l'Enregistreur de performances Windows (WPR) pour inclure des informations supplémentaires dans votre soumission au support Microsoft. WPR est un outil d'enregistrement puissant qui crée le suivi des événements pour les enregistrements Windows. 
...
4. Ensuite, téléchargez le [profil d'analyse microsoft Defender pour point](https://github.com/YongRhee-MDE/Scripts/blob/master/MDAV.wprp) de terminaison et enregistrez-le dans un dossier tel que `WD.wprp` `C:\temp` . 
...
L'outil en ligne *dewpr.exe* fait partie du système d'exploitation à partir de Windows 8. Pour collecter un suivi WPR à l'aide de l'outil en ligne de wpr.exe :
...
5. À l'invite avec élévation de droits, exécutez la commande suivante pour démarrer un suivi des performances de Microsoft Defender for Endpoint :
    ...
    >[!WARNING]
    >Si votre serveur Windows Server dispose de 64 Go ou de RAM ou plus, utilisez des profils et non des profils `WDForLargeServers.Light` `WDForLargeServers.Verbose` `WD.Light` `WD.Verbose` et, respectivement. Dans le cas contraire, votre système pourrait consommer une quantité élevée de mémoires ou de mémoires tampons de pool non pagyés, ce qui peut entraîner une instabilité du système.
...
7. À l'invite avec élévation de niveau élevé, exécutez la commande suivante pour arrêter le suivi des performances, en vous assurez de fournir des informations sur le problème et la façon dont vous avez reproduit le problème :

    ```console
    wpr.exe -stop merged.etl "Timestamp when the issue was reproduced, in HH:MM:SS format" "Description of the issue" "Any error that popped up"
    ```

8. Patientez jusqu'à ce que le suivi soit fusionné. 

9. Incluez à la fois le fichier et le dossier dans votre soumission au support Microsoft.
