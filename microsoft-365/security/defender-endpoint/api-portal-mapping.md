...
Comprendre les champs de données qui sont exposés dans le cadre de l’API de détections et la façon dont ils sont map Centre de sécurité Microsoft Defender.

>[!Note]
>- [Defender for Endpoint Alert](alerts.md) se compose d’une ou de plusieurs détections.
>- **Microsoft Defender ATP détection est** composée de l’événement suspect qui s’est produit sur l’appareil et de ses détails **d’alerte** associés.
