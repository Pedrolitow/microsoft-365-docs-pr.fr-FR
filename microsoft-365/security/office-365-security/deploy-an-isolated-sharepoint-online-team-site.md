...
## <a name="phase-1-create-and-populate-the-team-site-access-groups"></a>Phase 1 : Créer et remplir les groupes d’accès au site d’équipe
...
Si vous gérez des comptes d’utilisateurs et des groupes via Microsoft 365 et que vous souhaitez utiliser Windows PowerShell, faites une liste de leurs noms d’utilisateur principal (UPN) (exemple UPN : belindan@contoso.com).
...
Si vous gérez des comptes d’utilisateurs et des groupes via Microsoft 365 et que vous souhaitez utiliser PowerShell, faites une liste de leurs UPN. S’il existe un grand nombre de membres du site, vous pouvez enregistrer la liste de noms UPN dans un fichier texte et tous les ajouter avec une seule commande PowerShell.
...
1. Dans votre navigateur, go to the Azure portal at <https://portal.azure.com> and sign in with the credentials of an account that has been assigned with User Management Admin or Company Administrator role.
...
6. Répétez les étapes 3 à 5 pour vos groupes supplémentaires.

> [!NOTE]
> Vous devez utiliser le portail Azure pour créer les groupes afin qu’ils disposent de fonctionnalités Office activées. Si un site isolé SharePoint Online est ensuite configuré en tant que site hautement confidentiel avec une étiquette Azure Information Protection pour chiffrer des fichiers et attribuer des autorisations à des groupes spécifiques, les groupes autorisés doivent avoir été créés avec les fonctionnalités Office activées. Vous ne pouvez pas modifier le paramètre des fonctionnalités Office d’un groupe Azure AD après sa création.

Voici votre configuration résultante avec les trois groupes d’accès au site.
...
Si vous gérez des comptes d’utilisateurs et des groupes via les services de domaine Active Directory (AD DS), ajoutez des utilisateurs aux groupes d’accès appropriés à l’aide de vos procédures normales de gestion des utilisateurs et des groupes AD DS et attendez la synchronisation avec votre abonnement Microsoft 365.

Si vous gérez des comptes d’utilisateurs et des groupes via Office 365, vous pouvez utiliser le Centre d’administration Microsoft 365 ou PowerShell. Si vous avez des noms de groupes en double pour l’un des groupes d’accès, vous devez utiliser le Centre d’administration Microsoft 365.
...
- Le groupe Azure AD Administrateurs du site contient les comptes ou groupes d’utilisateurs administrateurs du site
- Le groupe Azure AD membres du site contient les comptes ou groupes d’utilisateurs membres du site
- Le groupe Azure AD des visiteurs du site contient les comptes d’utilisateurs ou les groupes qui peuvent uniquement afficher le contenu du site

Validez la liste des membres du groupe pour chaque groupe d’accès à l’aide du Centre d’administration Microsoft 365 ou du bloc de commandes PowerShell suivant :
...
Au cours de cette phase, vous allez créer le site SharePoint Online isolé et configurer les niveaux d’autorisation SharePoint Online par défaut pour utiliser vos nouveaux groupes d’accès Azure AD. Par défaut, les nouveaux sites d’équipe incluent un groupe Microsoft 365 et d’autres ressources connexes, mais dans ce cas, nous allons créer un site d’équipe sans groupe Microsoft 365. Cela permet de maintenir les autorisations entièrement via SharePoint.
...
2. Dans le Centre d’administration Microsoft 365, sous **Centres d’administration,** cliquez **sur SharePoint.**

3. Dans le Centre d’administration SharePoint, développez **Sites** et cliquez **sur Sites actifs.**
...
7. Dans **l’administrateur** principal, tapez le compte avec qui vous êtes connecté.
...
6. Dans le **volet Autorisations,** cliquez sur **Paramètres d’autorisations avancées.**

7. Sous **l’onglet Autorisations** de votre navigateur, cliquez sur **\<site name> Membres** dans la liste.
...
11. Cliquez **\<site name> sur Propriétaires** dans la liste.
...
15. Cliquez **\<site name> sur Visiteurs** dans la liste.
...
- Le **\<site name> groupe** SharePoint Propriétaires contient le groupe d’accès Administrateurs du site, dans lequel tous les membres ont le **niveau d’autorisation** Contrôle total.
- Le **\<site name> groupe** SharePoint Membres contient le groupe d’accès Membres du site, dans lequel tous les membres ont le **niveau d’autorisation** Modifier.
- Le **\<site name> groupe Visiteurs** SharePoint contient le groupe d’accès Visiteurs du site, dans lequel tous les membres ont le **niveau d’autorisation** Lecture.
- L’option permettant aux membres d’inviter d’autres membres ou non membres à demander l’accès au site est désactivée.

Voici votre configuration résultante avec les trois groupes SharePoint pour le site configuré pour utiliser les trois groupes d’accès, qui sont remplis avec des comptes d’utilisateur ou des groupes Azure AD.

![Configuration finale de votre site SharePoint Online isolé avec des groupes d’accès et des comptes d’utilisateur.](../../media/e7618971-06ab-447b-90ff-d8be3790fe63.png)
