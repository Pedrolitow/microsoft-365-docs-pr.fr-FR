...
3. Dans le **volant par** défaut qui s’affiche, configurez l’un des paramètres suivants :
   ...
   - **Liste d’adresses IP bloqués**: cliquez sur **Modifier.** Dans le volant de listes d’adresses **IP** bloqués qui s’affiche, entrez une adresse IP, une plage IP ou une adresse IP CIDR unique dans la zone Adresse ou plage d’adresses, comme décrit précédemment dans le paramètre Liste verte **IP.** 
