...
Cette série d’articles décrit un ensemble de configurations requises pour l’identité et l’accès aux appareils, ainsi qu’un ensemble de Azure Active Directory (Azure AD) accès conditionnel, Microsoft Intune et d’autres stratégies pour l’accès Confiance nulle à Microsoft 365  pour les applications et services cloud d’entreprise, les autres services SaaS et les applications locales publiés avec Azure AD Proxy d'application.
...
Regardez cette vidéo pour obtenir une vue d’ensemble rapide des configurations d’identité et d’accès aux appareils pour Microsoft 365 pour l’entreprise.
...
Ces recommandations sont destinées aux architectes d’entreprise et aux professionnels de l’informatique qui connaissent Microsoft 365 services de sécurité et de productivité cloud, notamment Azure AD (identité), Microsoft Intune (gestion des appareils) et Microsoft Purview Information Protection (protection des données). .

### <a name="customer-environment"></a>Environnement client

Les stratégies recommandées s’appliquent aux organisations d’entreprise fonctionnant entièrement dans le cloud Microsoft et pour les clients disposant d’une infrastructure d’identité hybride, qui est une forêt AD DS (Domain Services) Active Directory local synchronisée avec un locataire Azure AD.
...
Il est important d’utiliser des niveaux de protection cohérents sur vos identités, appareils et données. Par exemple, la protection pour les utilisateurs ayant des comptes&mdash; prioritaires, qu’ils soient cadres, dirigeants, responsables et autres&mdash;, inclut le même niveau de protection pour leurs identités, leurs appareils et les données auxquels ils accèdent. 
<!--
...
Microsoft 365 pour les entreprises est conçue pour permettre aux grandes organisations d’être créatives et de travailler ensemble en toute sécurité.
...
|[groupes Azure AD](/azure/active-directory/fundamentals/active-directory-manage-groups)|Les stratégies d’accès conditionnel, la gestion des appareils avec Intune et même les autorisations sur les fichiers et les sites de votre organisation s’appuient sur l’attribution à des comptes d’utilisateurs ou à des groupes Azure AD. Nous vous recommandons de créer Azure AD groupes qui correspondent aux niveaux de protection que vous implémentez. Par exemple, vos cadres supérieurs sont probablement des cibles de plus grande valeur pour les pirates informatiques. Par conséquent, il est judicieux d’ajouter les comptes d’utilisateur de ces employés à un groupe Azure AD et d’affecter ce groupe à des stratégies d’accès conditionnel et à d’autres stratégies qui appliquent un niveau de protection plus élevé pour l’accès.|Microsoft 365 E3 ou E5|
...
- [Configurer votre client pour une sécurité accrue](tenant-wide-setup-for-increased-security.md)
  ...
  Recommandations qui s’appliquent à la sécurité du point de départ pour votre locataire.
...
Avant de configurer et de déployer Confiance nulle configuration de l’identité et de l’accès aux appareils pour vos applications intégrées Azure AD, vous devez :
...
4. Configurez des stratégies d’accès conditionnel pour Microsoft 365 applications&mdash; cloud en tant que stratégies Microsoft Teams, Exchange et SharePoint&mdash; et Microsoft Defender for Cloud Apps.

Une fois que vous avez configuré Confiance nulle’accès aux identités et aux appareils, consultez le [guide de déploiement des fonctionnalités Azure AD](/azure/active-directory/fundamentals/active-directory-deployment-checklist-p2) pour obtenir une liste de vérification par phases des fonctionnalités supplémentaires à prendre en compte et [Azure AD gouvernance des identités](/azure/active-directory/governance/) pour protéger, surveiller et auditer l’accès.
