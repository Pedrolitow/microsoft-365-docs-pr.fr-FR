...
Pour confirmer que Application Guard pour Office est activé, lancez Word, Excel ou PowerPoint, puis ouvrez un document non approuvé. Par exemple, vous pouvez ouvrir un document téléchargé à partir d’Internet ou une pièce jointe d’une personne externe à votre organisation.
...
|---|---|
|N’utilisez pas Application Guard pour Office|L’activation de cette stratégie force Word, Excel et PowerPoint à utiliser le conteneur d’isolation en mode protégé au lieu d’Application Guard pour Office. Cette stratégie peut être utilisée pour désactiver temporairement Application Guard pour Office en cas de problème lors de son activation pour Microsoft Edge.|
|Configurer la pré-création du conteneur Application Guard pour Office|Cette stratégie détermine si le conteneur Application Guard pour Office, pour isoler les fichiers non approuvés, est précréé pour améliorer les performances d’exécution. Si vous activez ce paramètre, vous pouvez spécifier le nombre de jours pendant lesquels continuer la précréation d’un conteneur ou laisser le conteneur précréé intégré à Office créer le conteneur.
|N’autorisez pas le copier-coller pour les documents Office ouverts dans Application Guard pour Office|L’activation de cette stratégie empêche un utilisateur de copier et coller du contenu d’un document ouvert dans Application Guard pour Office vers un document ouvert en dehors de celui-ci.|
|Désactiver l’accélération matérielle dans Application Guard pour Office|Cette stratégie contrôle si Application Guard pour Office utilise l’accélération matérielle pour afficher des graphiques. Si vous activez ce paramètre, Application Guard pour Office utilise le rendu logiciel (UC) et ne charge aucun pilote graphique tiers ni n’interagit avec du matériel graphique connecté.
...
> * Désactiver l’accès à la caméra et au micro aux documents ouverts dans Application Guard pour Office
...
* Mises à jour à .NET peut entraîner l’échec de l’ouverture des fichiers dans Application Guard. En guise de solution de contournement, les utilisateurs peuvent redémarrer leur appareil lorsqu’ils rencontrent cet échec. En savoir plus sur le problème lors de la [réception d’un message d’erreur lors de la tentative d’ouverture de Protection d'application Windows Defender ou de Bac à sable Windows](https://support.microsoft.com/help/4575917/receiving-an-error-message-when-attempting-to-open-windows-defender-ap).
