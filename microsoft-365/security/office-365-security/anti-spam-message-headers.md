...
ms.locfileid: "43528290"
---
# <a name="anti-spam-message-headers-in-office-365"></a>En-têtes de messages anti-courrier indésirable dans Office 365
...
|CIP : \[adresse IP\]|Adresse IP de connexion. Vous pouvez utiliser cette adresse IP dans la liste d'adresses IP autorisées ou dans la liste d’adresses IP bloquées. Pour plus d’informations, voir [Configuration du filtrage des connexions dans Office 365](configure-the-connection-filter-policy.md).|
...
|IPV:CAL|Le message a ignoré le filtrage du courrier indésirable, car l’adresse IP source figure dans la liste d’adresses IP autorisées. Pour plus d’informations, voir [Configuration du filtrage des connexions dans Office 365](configure-the-connection-filter-policy.md).|
...
|SRV:BULK|Le message a été identifié comme courrier en bloc par le filtrage du courrier indésirable et le seuil de niveau de réclamation en bloc (BCL). Lorsque le paramètre _MarkAsSpamBulkMail_ est `On` (activé par défaut), un message électronique en nombre est marqué comme étant du courrier fortement suspecté d’être indésirable (SCL 9). Si vous souhaitez en savoir plus, consultez l’article [Configurer les stratégies anti-courrier indésirable dans Office 365](configure-your-spam-filter-policies.md).|
|X-CustomSpam : \[ASFOption\]|Le message correspondait à une option avancée de filtrage de courrier indésirable (ASF). Pour afficher la valeur de l’en-tête X pour chaque paramètre ASF, voir [Paramètres de filtre de courrier indésirable avancé dans Office 365](advanced-spam-filtering-asf-options.md).|
|
...
|---|---|
|**Champ d’en-tête**|**Description**|
|action|Indique l’action effectuée par le filtre de courrier indésirable en fonction des résultats de la vérification DMARC. Par exemple : <ul><li>**oreject** ou **o.reject** : signifie « override reject » (ignorer le rejet). Office 365 utilise cette action lorsqu’il reçoit un message qui échoue aux vérifications de DMARC et qui sont issus d’un domaine dont l’enregistrement TXT DMARC a pour stratégie p=reject (rejet). Au lieu de supprimer ou de rejeter le message, Office 365 le marque comme courrier indésirable. Pour obtenir des explications sur cette configuration d’Office 365, reportez-vous à la section [Gestion des messages électroniques entrants qui échouent aux vérifications de DMARC dans Office 365](use-dmarc-to-validate-email.md#inbounddmarcfail).</li><li>**pct.quarantine** : indique qu’un pourcentage inférieur à 100 % des messages n’ayant pas satisfait aux vérifications DMARC seront remis malgré tout. Cela signifie que le message n'a pas satisfait les vérifications DMARC et que la stratégie a été définie de manière à mettre les messages en quarantaine, mais que le champ pct n'a pas été défini sur 100 % et que le système a déterminé aléatoirement de ne pas appliquer l'action DMARC conformément à la stratégie du domaine spécifié.</li><li>**pct.reject** : indique qu’un pourcentage inférieur à 100 % des messages n'ayant pas satisfait aux vérifications DMARC seront remis malgré tout. Cela signifie que le message n'a pas satisfait les vérifications DMARC et que la stratégie a été définie de manière à rejeter les messages, mais que le champ pct n'a pas été défini sur 100 % et que le système a déterminé aléatoirement de ne pas appliquer l'action DMARC conformément à la stratégie du domaine spécifié.</li><li>**permerror** : indique qu’une erreur permanente s’est produite lors de l'évaluation DMARC, par exemple, qu’un enregistrement TXT DMARC mal formé a été détecté dans le système DNS. Toute tentative de renvoyer le message produira sans doute le même résultat. Essayez plutôt de contacter le propriétaire du domaine pour résoudre le problème.</li><li>**temperror** : indique qu’une erreur temporaire s’est produite lors de l’évaluation DMARC. Vous pouvez demander à l’expéditeur de renvoyer son message plus tard pour qu’il soit traité correctement.</li></ul>|
|compauth|Résultat de l’authentification composite. Utilisé par Office 365 pour combiner plusieurs types d’authentification tels que SPF, DKIM, DMARC ou toute autre partie du message pour déterminer si le message est authentifié ou non. Utilise le domaine From: comme base d’évaluation.<br/><br/>Les valeurs admises sont les suivantes :<ul><li>**pass** : le message a été authentifié de façon explicite (DMARC réussie ou la [meilleure hypothèse a réussi](https://blogs.msdn.microsoft.com/tzink/2015/05/06/what-is-dmarc-bestguesspass-in-office-365)), ou de façon implicite (le domaine expéditeur n’a pas publié d’enregistrement d’authentification de courrier, mais Office 365 a des signaux de serveur principal forts indiquant avec un haut degré de certitude que le message est probablement légitime).</li><li>**sofpass** : le message a été authentifié de façon implicite avec un degré de certitude faible à moyen. Le domaine expéditeur ne publie pas d’enregistrements d’authentification de courrier. Toutefois, Office 365 a des signaux de serveur principal indiquant que le message est légitime, mais que la force du signal est plus faible.</li><li>**fail** : le message n’a pas été authentifié de façon explicite (le domaine expéditeur a des enregistrements d’authentification de courrier) ou de façon implicite (le domaine expéditeur n’a pas d’enregistrement d’authentification de courrier et Office 365 a interpolé le résultat comme s’il avait des enregistrements publiés).</li><li>**none** : le message ne s’est pas authentifié (ou s’est authentifié mais ne s’est pas aligné), mais l’authentification composite n’a pas été appliquée en raison de la réputation de l’expéditeur ou d’autres facteurs.</li></ul>|
