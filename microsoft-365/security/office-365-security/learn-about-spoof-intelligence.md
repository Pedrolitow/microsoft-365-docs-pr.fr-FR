...
|Résultat de l’authentification  <br/> |Cette valeur est **transmise** si les vérifications d’authentification d’expéditeurs Exchange Online Protection (EoP) transmises par l’expéditeur, telles que SPF ou DKIM, **ont échoué** si l’expéditeur a échoué à la vérification de l’authentification d’expéditeurs EOP, ou **inconnu** si le résultat de ces vérifications n’est pas connu.  <br/> |
...
Même si la stratégie d’usurpation d’identité apparaît sous la stratégie anti-hameçonnage dans le centre de &amp; sécurité conformité, elle n’hérite pas du comportement par défaut du paramètre d’hameçonnage existant dans la configuration anti-courrier indésirable. Si vous avez des paramètres sous **hameçonnage** **anti-courrier indésirable** \> que vous souhaitez répliquer contre l’usurpation d’identité, vous devez créer une stratégie anti-hameçonnage, puis modifier la partie d’usurpation de la stratégie anti-hameçonnage en fonction de vos paramètres d’usurpation, comme décrit dans la section suivante, au lieu d’accepter les paramètres par défaut qui s’exécutent en arrière-plan.
...
- Passez en revue votre configuration DKIM (DomainKeys Identified Identified Mail). Vous devez utiliser DKIM en plus de SPF et DMARC pour empêcher les usurpateurs d’envoyer des messages qui semblent provenir de votre domaine. DKIM vous permet d'ajouter une signature numérique aux messages électroniques dans l'en-tête du message. Pour plus d’informations, consultez [la rubrique use DKIM pour valider les messages sortants envoyés à partir de votre domaine personnalisé dans Office 365](use-dkim-to-validate-outbound-email.md).
...
- Utilisez la cmdlet [Get-PhishFilterPolicy](https://docs.microsoft.com/powershell/module/exchange/advanced-threat-protection/get-phishfilterpolicy) dans Exchange Online PowerShell ou Exchange Online Protection PowerShell pour collecter des données détaillées sur des expéditeurs usurpés, générer des listes verte et rouge et vous aider à déterminer comment générer des enregistrements DNS SPF, DKIM et DMARC plus complets sans que votre courrier légitime ne soit intercepté dans les filtres de courrier indésirable externes. Pour plus d’informations, consultez la rubrique fonctionnement [de la protection contre l’usurpation d’identité dans Office 365](https://blogs.msdn.microsoft.com/tzink/2016/02/23/how-antispoofing-protection-works-in-office-365/).
