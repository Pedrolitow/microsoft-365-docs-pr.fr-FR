...
2. Vérifiez la configuration de votre contrôleur de domaine :
   ...
   - Vérifiez que [Microsoft Defender for Identity and Microsoft Cloud App Security Integration](https://docs.microsoft.com/cloud-app-security/mdi-integration) a été activé.
...
3. Vérifiez la configuration du périphérique de test :
   ...
   1. Le périphérique s’exécute avec Windows 10 version 1903 ou une version ultérieure.
...
#### <a name="alert-suspicious-process-injection-observed-source-microsoft-defender-for-endpoint-edr"></a>Alerte : l’injection de processus suspects a été observée (source : Microsoft Defender pour le point de terminaison EDR)
...
#### <a name="alert-unexpected-behavior-observed-by-a-process-run-with-no-command-line-arguments-source-microsoft-defender-for-endpoint-edr"></a>Alerte : comportement inattendu observé par un processus exécuté sans argument de ligne de commande (source : Microsoft Defender pour le point de terminaison EDR)
...
#### <a name="alert-user-and-ip-address-reconnaissance-smb-source-microsoft-defender-for-identity"></a>Alerte : utilisateur et recherche d’adresses IP (SMB) (source : Microsoft Defender pour l’identité)
...
#### <a name="go-hunting"></a>Trouver la chasse
...
3. Créer une requête qui commence par collecter des événements de messagerie.
   ...
   1. Dans le volet requête, sélectionnez Nouveau.

   1. Double-cliquez sur la table EmailEvents dans le schéma.

      ```console
      EmailEvents
      ```

   1. Définissez le délai sur les dernières 24 heures. En supposant que le courrier électronique que vous avez envoyé lors de l’exécution de la simulation ci-dessus était au cours des dernières 24 heures, sinon, modifiez le délai.
   ...
   1. Exécutez la requête. Vous pouvez avoir un grand nombre de résultats en fonction de l’environnement du pilote.
   ...
   1. Cliquez sur les lignes résultantes à partir de la requête afin de pouvoir inspecter l’enregistrement.
