...
<p>Vérifiez que vos applications sont empaquetées et prêtes pour l’installation automatisée. Découvrez comment l’empaquetage Démarrer en un clic avec Microsoft 365 Apps pour entreprise vous offre de nouvelles options pour configurer, livrer et actualiser vos applications Office.</p></td>
...
Une autre méthode populaire de remise application est IMAGERIE PC. Dans ce cas, les applications sont soit installées via la séquence des tâches ou manuellement sur un échantillon PC, une image de système est capturée avec les applications requises déjà installées. L’approche d’images pour concevoir et capturer peut faire gagner du temps lors de la mise en service de nouveaux PC mais n’oubliez pas que les systèmes d’exploitation et les applications au sein de l’image peuvent devenir obsolètes rapidement. Le modèle de mise à jour cumulative dans Windows 10 et Microsoft 365 Apps pour entreprise aide à résoudre ce problème, mais ne l’élimine pas complètement. C’est pourquoi nous vous recommandons une approche d’images fines, où vos applications sont installées à partir de l’extérieur de l’image au moment du déploiement.
...
Microsoft 365 Apps pour entreprise est installé au moyen de la technologie « Démarrer en un clic » qui remplace le package basé sur MSI dans toutes les versions de la prochaine publication 2019 Office pour Windows. Il affiche un nombre d’avantages, y compris des installations plus rapides, une mise à jour rapide et plus efficace et une désinstallation plus nette. 
...
Si vous utilisez l’outil Gestionnaire de configuration, vous pouvez toujours l’utiliser pour le déploiement massif de Microsoft 365 Apps pour entreprise. L’outil Gestionnaire de configuration (branche actuelle) assure la prise en charge native de l’outil de personnalisation Office mis à jour, la personnalisation de package pour Démarrer en un clic au moment de l’installation ainsi que la prise en charge native pour la gestion des mises à jour logicielles après l’installation.
...
## <a name="step-4-user-files-and-settings"></a>[Étape 4 : fichiers et paramètres utilisateur](https://aka.ms/mdd4)
