...
Au moment de l’écriture, les plages d’adresses IP correspondant à ces points de terminaison sont les suivantes. Il est **très fortement** conseillé d'utiliser un [script tel que cet](https://github.com/microsoft/Office365NetworkTools/tree/master/Scripts/Display%20URL-IPs-Ports%20per%20Category) exemple, le [service web d’Office 365 et d’URL](microsoft-365-ip-web-service.md) ou la [page d’URL/IP](urls-and-ip-address-ranges.md) pour rechercher les mises à jour lors de l’application de la configuration, et mettre en place une stratégie pour le faire régulièrement.
...
Une priorité doit être accordée au points de terminaison marqués **Optimiser**, car ceux-ci vous permettront de bénéficier d’un niveau de travail optimal. Toutefois, si vous le souhaitez, les points de terminaison autoriser marqués sont requis pour que le service fonctionne et que des IP soient fournis pour les points de terminaison qui peuvent être utilisés si nécessaire.
...
**Pas de**, ce n’est pas le cas. Il fonctionne uniquement sur le client Microsoft Teams version 1.3.00.13565 ou supérieure. Cette version inclut des améliorations dans la façon dont le client détecte les chemins d’accès réseau disponibles.
