...
4. Votre équipe a lu l’ensemble des conseils publics et de la documentation [https://aka.ms/expressrouteoffice365](./azure-expressroute.md)sur , [https://aka.ms/ert](https://aka.ms/ert)et a vu la série de formations [Azure ExpressRoute pour Office 365](https://channel9.msdn.com/series/aer) sur Channel 9 pour comprendre les détails techniques essentiels, notamment :
...
- Collectez de la documentation sur la topologie réseau existante affichant les détails de votre structure de réseau principal et topologie wan interne, la connectivité des sites satellites, la connectivité utilisateur de dernière distance, le routage vers les points de sortie du périmètre réseau et les services proxy.
  ...
  - Identifiez les points de terminaison du service entrant sur les diagrammes réseau à Office 365 et d’autres services Microsoft, en affichant les chemins de connexion ExpressRoute proposés et Internet.
...
|**Contrôle de symétrie de chemin d’accès** <br/> |**Méthode** : Chemin d’accès **Internet** NAT source : connexions entrantes NAT source vers 192.168.5.5 **ExpressRoute** : connexions NAT source vers 192.168.1.0 (Chicago) et 192.168.2.0 (Dallas)  <br/> |
...
|**Point de Office 365 public (destination)** <br/> |SharePoint Online (adresses IP)  <br/> |
|**Entrée DNS publique (Internet)** <br/> |\*.sharepoint.com (et plus de FQDN)  <br/> |
|**CDN références** <br/> |cdn.sharepointonline.com (et plus de FQDN) : adresses IP conservées par CDN fournisseurs)  <br/> |
...
6. Identifiez chaque emplacement où les personnes accèderont Office 365 à partir d’expressroute et résaiez les emplacements de rendez-vous qui seront utilisés pour ExpressRoute.
...
Pour le trafic sortant, les personnes accèdent Office 365'une des trois manières possibles :
...
Il existe souvent plusieurs emplacements de rendez-vous qui peuvent être sélectionnés dans une région avec une proximité relative avec vos utilisateurs. Remplissez le tableau suivant pour guider vos décisions.
...
Votre plan d’implémentation doit englober à la fois les détails techniques de la configuration d’ExpressRoute et les détails de la configuration de toutes les autres infrastructures de votre réseau, tels que les suivants.
...
- Planifiez les modifications des enregistrements DNS, y compris [les entrées de l’infrastructure de stratégie de l’expéditeur](../security/office-365-security/set-up-spf-in-office-365-to-help-prevent-spoofing.md) .
...
- Planifiez le routage lan client de l’utilisateur final, comme la configuration d’un fichier [PAC/WPAD](./managing-office-365-endpoints.md), de l’itinéraire par défaut, des serveurs proxy et des annonces d’itinéraire BGP.
...
Si votre conception réseau ne répond pas à ces exigences, vos utilisateurs risquent de rencontrer des échecs de connectivité à Office 365 et à d’autres services Microsoft en raison de l’acheminement d’un holage noir ou d’un routage asymétrique. Cela se produit lorsque les demandes à services Microsoft sont acheminées via ExpressRoute, mais que les réponses sont réacheminées sur Internet, ou vice versa, et que les réponses sont abandonnées par des périphériques réseau avec état tels que des pare-feu.
...
2. Toutes les résolutions de noms DNS Office 365 que les services exécutent pour résoudre les points de terminaison locaux se produisent à l’aide du DNS public. Cela signifie que vous devez inscrire le FQDN des points de terminaison de service entrants sur les mappages IP sur Internet.
...
- Assurez-vous que vous êtes abonné au flux RSS pour Office 365 notifications de point de terminaison afin de gérer les configurations de routage ou proxy.
...
2. Validez les 500 Office 365 et les annonces d’adresses IP CRM Online reçues par votre routeur local.
...
La QoS est nécessaire pour obtenir les avantages de la voix et des réunions pour Skype Entreprise Online. Vous pouvez configurer QoS après vous être assuré que la connexion réseau ExpressRoute ne bloque aucun autre accès Office 365 service. La configuration de QoS est décrite dans l’article [ExpressRoute et QoS dans Skype Entreprise Online](https://support.office.com/article/ExpressRoute-and-QoS-in-Skype-for-Business-Online-20c654da-30ee-4e4f-a764-8b7d8844431d).
