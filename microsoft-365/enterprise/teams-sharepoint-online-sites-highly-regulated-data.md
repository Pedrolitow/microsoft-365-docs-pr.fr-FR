...
- Verrouillez le site pour empêcher :
  - L’accès aux utilisateurs qui ne sont pas membres du groupe Microsoft 365 pour le site.
  - les membres du site d’octroyer l’accès à d’autres personnes ;
  - l’accès au site par les personnes non membres.
- Configurez une étiquette de rétention pour vos sites SharePoint comme moyen par défaut pour empêcher les utilisateurs d’envoyer des fichiers à l’extérieur de l’organisation.
...
| Verrouiller le site | Autorisations de sites d’équipe SharePoint et de groupes Microsoft 365 |
| Étiqueter les fichiers du site | Étiquettes de rétention Microsoft 365 |
| Bloquer les utilisateurs lors de l’envoi de fichiers à l’extérieur de l’organisation | Stratégie de prévention des pertes de données (DLP) |
| Chiffrer tous les fichiers du site | Sous-étiquettes ou étiquettes de confidentialité de Microsoft 365 |
| Ajouter des autorisations aux fichiers du site | Sous-étiquettes ou étiquettes de confidentialité de Microsoft 365 |
|||
...
<a name="poster"></a> Pour un résumé d’une page de ce scénario, voir l’[affiche Sites SharePoint pour les données hautement réglementées](../media/teams-sharepoint-online-sites-highly-regulated-data/SharePointSitesHighlyRegulatedData.pdf).
...
- L'étiquette de rétention à attribuer à la partie Documents du site et les stratégies DLP liées à l'étiquette
- Les paramètres d’une sous-étiquette de confidentialité que les utilisateurs appliquent aux fichiers hautement sensibles stockés sur le site

Une fois ces paramètres déterminés, utilisez-les pour configurer le site à la phase 2. 

### <a name="step-1-microsoft-365-retention-labels-and-dlp-policies"></a>Étape 1 : étiquettes de rétention Microsoft 365 et stratégies DLP

Lorsqu’elles sont appliquées à la partie Documents du site d’équipe SharePoint, les étiquettes de rétention fournissent une méthode par défaut de classification de toutes les fichiers stockées sur le site.
 
En ce qui concerne les sites SharePoint pour les données hautement réglementées, vous devez déterminer les étiquettes de rétention à utiliser.

En ce qui concerne les considérations relatives à la conception d’étiquettes, consultez la rubrique [Classification et étiquettes Microsoft 365](https://docs.microsoft.com/office365/securitycompliance/secure-sharepoint-online-sites-and-files#office-365-retention-labels).
...
En ce qui concerne les sites SharePoint, vous devez configurer une stratégie DLP pour l’étiquette de rétention attribuée au site afin de bloquer les utilisateurs lorsqu’ils essaient de partager des fichiers avec des utilisateurs externes. 

### <a name="step-2-your-microsoft-365-sensitivity-sublabel"></a>Étape 2 : votre sous-étiquette de confidentialité Microsoft 365

Pour fournir un chiffrement et un ensemble d’autorisations à vos fichiers les plus sensibles, les utilisateurs doivent appliquer une sous-étiquette ou une étiquette de confidentialité. Une sous-étiquette se trouve sous une étiquette existante. 
...
- L’étiquette de rétention appropriée et la stratégie DLP associée à l’étiquette
- Les paramètres de la sous-étiquette de confidentialité comprenant le chiffrement et les autorisations
...
### <a name="step-1-create-a-private-sharepoint-team-site-with-owners-and-members-of-the-corresponding-microsoft-365-group"></a>Étape 1 : créer un site d’équipe SharePoint privé avec les propriétaires et les membres du groupe Microsoft 365 correspondant
...
### <a name="step-3-configure-the-site-for-a-retention-label"></a>Étape 3 : configurer le site pour une étiquette de rétention

Suivez les instructions mentionnées dans [Protéger les fichiers SharePoint avec les étiquettes et la protection contre la perte de données (DLP)](https://docs.microsoft.com/office365/enterprise/protect-sharepoint-online-files-with-office-365-labels-and-dlp) pour :
...
#### <a name="step-4-create-a-sensitivity-sublabel-for-the-site"></a>Étape 4 : créer une sous-étiquette de confidentialité pour le site
...
- Une étiquette de rétention attribuée à la partie Documents du site SharePoint
- Une stratégie DLP pour l’étiquette de rétention
- Une sous-étiquette de confidentialité que les utilisateurs peuvent appliquer aux fichiers les plus sensibles stockés sur le site, qui chiffre le fichier et autorise uniquement l’accès de co-création pour les membres du groupe de site d’équipe 
...
## <a name="how-the-contoso-corporation-used-a-sharepoint-site-for-highly-regulated-data"></a>Comment Contoso Corporation a utilisé un site SharePoint pour des données hautement réglementées
