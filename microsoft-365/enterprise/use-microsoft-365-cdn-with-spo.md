...
Avant d’apporter des modifications aux paramètres CDN du locataire, vous devez récupérer l’état actuel de la configuration du CDN privé dans votre locataire Office 365. Connectez-vous à votre locataire à l’aide de SharePoint Online Management Shell :
...
Consultez [les origines CDN par défaut](use-microsoft-365-cdn-with-spo.md#default-cdn-origins) pour plus d’informations sur les origines qui sont approvisionnées par défaut lorsque vous activez le CDN Office 365 et l’impact potentiel de l’omission de la configuration des origines par défaut.
...
Par exemple, pour permettre au CDN d’héberger des fichiers .css et .png, vous devez entrer la commande :
...
### <a name="change-the-list-of-site-classifications-you-want-to-exclude-from-the-office-365-cdn-optional"></a>Modifier la liste des classifications de sites à exclure du CDN Office 365 (facultatif)
...
La propriété _ExcludeRestrictedSiteClassifications_ contient les classifications de site que vous souhaitez exclure du CDN. Par exemple, vous pouvez exclure les sites marqués comme **confidentiels** afin que le contenu des sites avec cette classification appliquée ne soit pas servi à partir du CDN.
...
> [!NOTE]
> Dans les origines privées, les ressources partagées à partir d’une origine doivent avoir une version majeure publiée avant d’être accessibles à partir du CDN.
...
Normalement, ces origines sont configurées pour vous par défaut lorsque vous activez le CDN Office 365. Toutefois, si vous souhaitez les activer manuellement, procédez comme suit.
...
Vous ne pouvez pas modifier une origine que vous avez créée. Au lieu de cela, supprimez l’origine, puis ajoutez-en une nouvelle. Pour plus d’informations, consultez [Pour supprimer une origine du CDN Office 365](use-microsoft-365-cdn-with-spo.md#Office365CDNforSPORemoveOriginPosh) et [Pour ajouter une origine pour vos ressources](use-microsoft-365-cdn-with-spo.md#Office365CDNforSPOOriginPosh).
...
Avant d’apporter des modifications aux paramètres CDN du locataire, vous devez récupérer l’état actuel de la configuration du CDN privé dans votre locataire Office 365. Connectez-vous à votre locataire à l’aide de PnP PowerShell :
...
Pour plus d’informations sur cette applet de commande, consultez [Set-PnPTenantCdnEnabled](https://pnp.github.io/powershell/cmdlets/Set-PnPTenantCdnEnabled.html).
...
Pour plus d’informations sur ces applets de commande, consultez [Set-PnPTenantCdnPolicy](https://pnp.github.io/powershell/cmdlets/Set-PnPTenantCdnPolicy.html) et [Get-PnPTenantCdnPolicies](https://pnp.github.io/powershell/cmdlets/Get-PnPTenantCdnPolicies.html).
...
Pour plus d’informations sur cette commande et sa syntaxe, consultez [Add-PnPTenantCdnOrigin](https://pnp.github.io/powershell/cmdlets/Add-PnPTenantCdnOrigin.html).
...
Pour plus d’informations sur l’utilisation de cette applet de commande, consultez [Remove-PnPTenantCdnOrigin](https://pnp.github.io/powershell/cmdlets/Remove-PnPTenantCdnOrigin.html).

<a name="Office365CDNforSPOModifyOriginPnPPosh"> </a>
#### <a name="modify-an-origin-in-the-office-365-cdn"></a>Modifier une origine dans le CDN Office 365

Vous ne pouvez pas modifier une origine que vous avez créée. Au lieu de cela, supprimez l’origine, puis ajoutez-en une nouvelle. Pour plus d’informations, consultez [Pour supprimer une origine du CDN Office 365](use-microsoft-365-cdn-with-spo.md#Office365CDNforSPORemoveOriginPnPPosh) et [Pour ajouter une origine pour vos ressources](use-microsoft-365-cdn-with-spo.md#Office365CDNforSPOOriginPnPPosh).
...
Pour activer la Office 365 CDN SharePoint, exécutez :
...
Pour plus d’informations sur les origines approvisionnées par défaut lorsque vous activez le CDN Office 365, consultez les [origines CDN par défaut](use-microsoft-365-cdn-with-spo.md#default-cdn-origins).

### <a name="add-an-office-365-cdn-origin"></a>Ajouter une origine CDN Office 365

> [!IMPORTANT]
> Vous ne devez jamais placer les ressources considérées comme sensibles à votre organisation dans une bibliothèque de documents SharePoint configurée en tant qu’origine publique.
...
### <a name="remove-an-office-365-cdn-origin"></a>Supprimer une origine CDN Office 365
...
> [!NOTE]
> La suppression d’une origine CDN n’affecte pas les fichiers stockés dans une bibliothèque de documents correspondant à cette origine. Si ces ressources ont été référencées à l’aide de leur URL SharePoint, SharePoint revient automatiquement à l’URL d’origine pointant vers la bibliothèque de documents. Toutefois, si des ressources ont été référencées à l’aide d’une URL CDN publique, la suppression de l’origine interrompt le lien et vous devrez les modifier manuellement.

### <a name="modify-an-office-365-cdn-origin"></a>Modifier une origine CDN Office 365
