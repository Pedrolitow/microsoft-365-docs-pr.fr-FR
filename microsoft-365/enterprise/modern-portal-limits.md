---
title: SharePoint limites de site du portail moderne En ligne
ms.author: kvice
...
ms.locfileid: "65091146"
---
# <a name="sharepoint-online-modern-portal-site-limits"></a>SharePoint limites de site du portail moderne En ligne

Cet article fournit des recommandations en matière de performances pour les sites du portail moderne dans SharePoint Online. Suivez les instructions de cet article pour optimiser les performances du site portail moderne et éviter les problèmes de performances courants.
...
- Décharger l’hébergement de fichiers statiques courants tels que des images, des feuilles de style et des scripts pour CDN
- Limiter les appels aux points de terminaison SharePoint et externes uniquement à ce qui est nécessaire
- Éviter les demandes en double pour le même contenu

La plupart des instructions de cet article se concentrent sur la réduction et l’optimisation des appels à SharePoint Online. L’exécution d’appels répétitifs chaque fois qu’une page est chargée aura un impact sur les performances des utilisateurs, car les informations seront récupérées du service chaque fois, même si elles n’ont pas changé. Par conséquent, les demandes à SharePoint peuvent être classées en tant qu’appels courants pour tous les utilisateurs ou appels requis pour chaque utilisateur individuel. Les résultats de ces deux catégories d’appels doivent être mis en cache pour optimiser l’expérience utilisateur.

>[!NOTE]
>Utilisez [l’outil Diagnostics de page pour SharePoint](./page-diagnostics-for-spo.md) comme point de départ pour analyser des métriques de performances spécifiques sur SharePoint pages de site En ligne.
...
|Composants WebPart sur une page  <br/> |20 par page  <br/> |Nous vous recommandons d’utiliser au moins 20 composants WebPart par page, y compris les composants WebPart Microsoft et les composants WebPart personnalisés. <br/> Pour plus d’informations, consultez [Optimiser les performances des composants WebPart dans SharePoint pages de site modernes en ligne](modern-web-part-optimization.md).  <br/> |
|Composants WebPart dynamiques sur une page  <br/> |4 par page  <br/> |Les composants WebPart dynamiques qui effectuent une ou plusieurs requêtes pour SharePoint pour extraire les données les plus récentes doivent être limités à 4 par page. Le composant _WebPart Actualités_ est un exemple de composant WebPart dynamique. <br/> Pour plus d’informations, consultez [Optimiser les performances des composants WebPart dans SharePoint pages de site modernes en ligne](modern-web-part-optimization.md).    <br/> |
