...
Vous utilisez Azure AD Connecter pour synchroniser vos AD DS avec le client Azure AD client de votre Microsoft 365 client. Pour plus d’informations, [voir Intégration de vos identités locales à Azure AD](/azure/architecture/reference-architectures/identity/azure-ad).
  
Azure AD Connecter synchronise l’UPN et le mot de passe de vos utilisateurs afin qu’ils se connectent avec les mêmes informations d’identification qu’ils utilisent en local. Toutefois, Azure AD Connecter synchronise uniquement les utilisateurs avec les domaines qui sont vérifiés par Microsoft 365. Cela signifie que le domaine est également vérifié par Azure AD, car Microsoft 365 identités sont gérées par Azure AD. En d’autres termes, le domaine doit être un domaine Internet valide (par exemple, .com, .org, .net, .us). Si votre AD DS interne utilise uniquement un domaine non routable (par exemple, « .local »), cela ne peut pas correspondre au domaine vérifié dont vous avez besoin pour votre client Microsoft 365. Vous pouvez résoudre ce problème en modifiant votre domaine principal dans votre AD DS local ou en ajoutant un ou plusieurs suffixes UPN.
  
### <a name="change-your-primary-domain"></a>Modifier votre domaine principal

Modifiez votre domaine principal en un domaine que vous avez vérifié dans Microsoft 365, par exemple, contoso.com. Chaque utilisateur qui possède le domaine contoso.local est ensuite mis à jour contoso.com. Il s’agit toutefois d’un processus impliqué et une solution plus facile est décrite dans la section suivante.
...
1. Sur le contrôleur de domaine AD DS, dans le Gestionnaire de serveur, sélectionnez **Outils** \> **Domaines et trusts Active Directory**.
...
3. Sous **l’onglet** Compte, dans la liste du suffixe UPN, choisissez le nouveau suffixe UPN, puis **choisissez OK**.
...
Si vous avez de nombreux comptes d’utilisateur à mettre à jour, il est plus facile d’utiliser PowerShell. L’exemple suivant utilise les cmdlets [Get-ADUser](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/ee617241(v=technet.10)) et [Set-ADUser](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/ee617215(v=technet.10)) pour modifier tous les suffixes contoso.local en contoso.com dans AD DS. 
...
Pour [en savoir plus sur l’utilisation Windows PowerShell AD](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/ee617195(v=technet.10)) DS, voir le module Windows PowerShell Active Directory.