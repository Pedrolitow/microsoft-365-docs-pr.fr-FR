---
title: Configurer un connecteur pour archiver les données ICE Connect Chat dans Microsoft 365
description: Découvrez comment configurer et utiliser un connecteur 17a-4 ICE Connect Chat DataParser pour importer et archiver les données ice Connect Chat dans Microsoft 365.
f1.keywords:
- NOCSH
ms.author: robmazz
author: robmazz
...
- purview-compliance
- data-connectors
ms.openlocfilehash: bc78ccb874082d458f312c5f44c4c122e20a0297
...
---
# <a name="set-up-a-connector-to-archive-ice-connect-chat-data"></a>Configurer un connecteur pour archiver les données ice Connect Chat

Utilisez [ice DataParser](https://www.17a-4.com/ice-dataparser/) de 17a-4 LLC pour importer et archiver des données à partir d’ICE Connect Chat dans les boîtes aux lettres des utilisateurs de votre organisation Microsoft 365. DataParser inclut un connecteur ICE Chat configuré pour capturer des éléments à partir d’une source de données tierce et importer ces éléments dans Microsoft 365. Le connecteur ICE DataParser convertit les données ICE Connect Chat au format de message électronique, puis importe ces éléments dans les boîtes aux lettres des utilisateurs dans Microsoft 365.

Une fois que les données ICE Connect Chat sont stockées dans des boîtes aux lettres utilisateur, vous pouvez appliquer des fonctionnalités Microsoft Purview telles que la conservation pour litige, eDiscovery, les stratégies de rétention et les étiquettes de rétention, ainsi que la conformité des communications. L’utilisation d’un connecteur ICE DataParser pour importer et archiver des données dans Microsoft 365 peut aider votre organisation à rester conforme aux stratégies gouvernementales et réglementaires.
...
![Flux de travail d’archivage pour les données ice Connect Chat de 17a-4.](../media/ICEChatDataParserConnectorWorkflow.png)

1. Votre organisation utilise 17a-4 pour configurer ice DataParser.

2. Régulièrement, les éléments ICE Connect Chat sont collectés par dataParser. DataParser convertit également le contenu d’un message au format de message électronique.
...
4. Un sous-dossier dans le dossier Boîte de réception nommé **ICE DataParser** est créé dans les boîtes aux lettres utilisateur, et les éléments ICE Connect Chat sont importés dans ce dossier. Le connecteur détermine la boîte aux lettres dans laquelle importer les éléments à l’aide de la valeur de la propriété *Email*. Chaque élément ICE Connect Chat contient cette propriété, qui est renseignée avec l’adresse e-mail de chaque participant.
...
1. Accédez à <https://compliance.microsoft.com> , puis sélectionnez **Connecteurs** >  de données **ICE DataParser**.
...
5. Connectez-vous à votre compte 17a-4 et suivez les étapes décrites dans l’Assistant Connexion ICE DataParser.

## <a name="step-2-configure-the-ice-dataparser-connector"></a>Étape 2 : Configurer le connecteur ICE DataParser

Utilisez la prise en charge 17a-4 pour configurer le connecteur Ice DataParser.

## <a name="step-3-map-users"></a>Étape 3 : Mapper les utilisateurs

Le connecteur ICE DataParser mappera automatiquement les utilisateurs à leurs adresses e-mail Microsoft 365 avant d’importer des données dans Microsoft 365.
...
2. Sélectionnez l’onglet **Connecteurs** , puis sélectionnez le connecteur ICE DataParser que vous avez créé pour afficher la page de menu volant, qui contient les propriétés et les informations sur le connecteur.
