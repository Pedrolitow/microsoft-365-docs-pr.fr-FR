...
Pour vous préparer, vous devez assembler une feuille de calcul Excel spécialement mise en forme pour importer les données de modèle nécessaires. Les fichiers Excel suivent le même format décrit dans les données du modèle d’évaluation de format avec [Excel, mais](compliance-manager-templates-format-excel.md) il existe des exigences spéciales pour les extensions. Consultez ces points supplémentaires pour éviter les erreurs :
