...
La vue d’ensemble suivante explique le processus d’utilisation d’un connecteur de données pour archiver des données InvestEdge dans Microsoft 365.

![Flux de travail d’archivage pour les données InvestEdge comprises entre 17a et 4.](../media/InvestEdgeDataParserConnectorWorkflow.png)

1. Votre organisation utilise 17a-4 pour configurer InvestEdge DataParser.

2. Régulièrement, les éléments InvestEdge sont collectés par le DataParser. DataParser convertit également le contenu d’un message au format de message électronique.

3. Le connecteur InvestEdge DataParser que vous créez dans le portail de conformité Microsoft Purview se connecte à DataParser et transfère les messages vers un emplacement de stockage Azure sécurisé dans le cloud Microsoft.
...
1. Accédez à<https://compliance.microsoft.com>, puis sélectionnez **Connecteurs** >  de données **InvestEdge DataParser**.

2. Dans la page de description du produit **InvestEdge DataParser** , sélectionnez **Ajouter un connecteur**.
...
Utilisez la prise en charge 17a-4 pour configurer le connecteur InvestEdge DataParser.

## <a name="step-3-map-users"></a>Étape 3 : Mapper les utilisateurs

Le connecteur InvestEdge DataParser mappera automatiquement les utilisateurs à leurs adresses e-mail Microsoft 365 avant d’importer des données dans Microsoft 365.
...
2. Sélectionnez l’onglet **Connecteurs** , puis sélectionnez le connecteur InvestEdge DataParser que vous avez créé pour afficher la page de menu volant, qui contient les propriétés et les informations sur le connecteur.
