...
- Windows’infrastructure de classification des fichiers du serveur (FCI)
...
Avant de pouvoir utiliser une propriété Windows Server FCI ou une autre propriété dans une stratégie DLP, vous devez créer une propriété gérée dans le Centre d’administration <a href="https://go.microsoft.com/fwlink/?linkid=2185219" target="_blank">SharePoint</a>. Voici pourquoi.
...
2. Dans le navigation de gauche, sélectionnez **Centres d’administration** \> **SharePoint**. Vous êtes maintenant dans le centre <a href="https://go.microsoft.com/fwlink/?linkid=2185219" target="_blank">d SharePoint’administration.</a>

3. Dans le navigation de gauche, choisissez **recherche dans** \> la page \> **d’administration de la** recherche **Gérer le schéma de recherche**.
...
Dans cet exemple, une organisation utilise l’IFCI sur ses serveurs de fichiers basés sur un serveur Windows ; plus précisément, elle utilise la propriété de classification FCI nommée **Informations** d’identification personnelle avec les valeurs possibles de **piI** **Élevé, Modéré****, Faible****,** **Public** et Non. À présent, ils souhaitent utiliser leur classification FCI existante dans leurs stratégies DLP dans Office 365.
...
Ensuite, ils créent une stratégie DLP avec deux règles qui utilisent toutes deux la condition **Propriétés document contiennent l’une des valeurs suivantes** :
...
Les propriétés **de document** &amp; de condition contiennent l’une de ces valeurs n’est temporairement pas disponible dans l’interface utilisateur du Centre de conformité de sécurité, mais vous pouvez toujours utiliser cette condition à l’aide de PowerShell. Vous pouvez utiliser  `New\Set\Get-DlpCompliancePolicy` les cmdlets pour utiliser une stratégie DLP et  `New\Set\Get-DlpComplianceRule` utiliser les cmdlets  `ContentPropertyContainsWords` avec le paramètre pour ajouter la condition Que les propriétés document contiennent l’une de **ces valeurs**.

Pour plus d’informations sur ces cmdlets, voir [cmdlets &amp; du Centre de conformité de sécurité](/powershell/exchange/exchange-online-powershell).
...
3. Créez les deux règles décrites `New-DlpComplianceRule`ci-dessus à l’aide de  , où une règle est pour la valeur Faible, et une autre règle pour les valeurs **Élevée** **et** Modéré.
...
2. Sous **Recherche**, sélectionnez Site de réindexation **de disponibilité** \> hors **connexion et de recherche**.
