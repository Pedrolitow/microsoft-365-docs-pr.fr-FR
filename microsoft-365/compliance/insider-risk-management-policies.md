...
Consultez la vidéo [Configuration](https://www.youtube.com/watch?v=kudK5ajZTUo) des stratégies de gestion des risques internes pour obtenir une vue d’ensemble de la façon dont les stratégies créées à l’aide de modèles de stratégie intégrés peuvent vous aider à agir rapidement sur les risques potentiels.
...
- **État** : l’état d’intégrité de chaque stratégie. Affiche le nombre de recommandations et d’avertissements de la stratégie, ou un état *Intègre* pour les stratégies sans problème.  Vous pouvez sélectionner la stratégie pour voir les détails de l’état d’état d’état des avertissements ou recommandations.
...
### <a name="general-patient-data-misuse-preview"></a>Utilisation incorrecte des données générales des patients (aperçu)
...
| :------------------ | :--------------------------------- | :---------------- |
| **Vol de données par des employés quittant votre organisation** | Indicateur de date de départ ou de résiliation à partir d’un connecteur RH Azure Active Directory suppression d’un compte | (facultatif) Connecteur Microsoft 365 RH configuré pour les indicateurs de date de démission ou de licenciement |
...
| **Utilisation incorrecte des données générales des patients** | Défense contre les contrôles de sécurité des systèmes EMR <br><br> Indicateurs de correspondance des adresses des utilisateurs et des patients des systèmes RH | Indicateurs d’accès aux soins de santé sélectionnés dans les paramètres de stratégie ou de risques internes <br><br> Microsoft 365 HR configuré pour la correspondance d’adresses <br><br> Connecteur Microsoft Santé ou Santé configuré |
...
- **Collection** : ces signaux de catégorie se concentrent sur les activités de téléchargement par des utilisateurs de stratégie dans l’étendue. Certains exemples d’activités de cette catégorie seraient le téléchargement de fichiers à partir SharePoint sites ou le déplacement de fichiers dans un dossier compressé.
...
- **Dissimulation** : ces signaux de catégorie se concentrent sur le masquage des activités à risque par des utilisateurs de stratégie dans l’étendue. Il peut s’agit, par exemple, de renommer des fichiers sur un appareil ou de supprimer ou de dégrader des étiquettes de niveau de SharePoint fichiers.
...
Pour afficher rapidement l’état d’intégrité d’une stratégie, naviguez vers l’onglet Stratégie et la colonne État. Les options d’état de stratégie suivantes s’offrent à vous pour chaque stratégie :
...
| Les alertes Microsoft Defender pour les points de terminaison ne sont pas partagées avec le centre de conformité | -Violations générales de la stratégie de sécurité <br> -Violations de stratégie de sécurité par des utilisateurs quittant l’entreprise <br> -Violations de stratégie de sécurité par des utilisateurs mécontents <br> -Violations de la stratégie de sécurité par des utilisateurs prioritaires | Les alertes Microsoft Defender pour point de terminaison ne sont pas partagées avec le Centre de conformité. <br><br> Configurez le partage des alertes de Microsoft Defender pour point de terminaison. |
| Vous approchez de la limite maximale d’utilisateurs activement marqués pour ce modèle de stratégie. | Tous les modèles de stratégie | Chaque modèle de stratégie a un nombre maximum d’utilisateurs dans l’étendue. Consultez les détails de la section limite du modèle. <br><br> Examinez les utilisateurs de l’onglet Utilisateurs et supprimez tous les utilisateurs qui n’ont plus besoin d’être marqués. |
| Le déclenchement d’un événement se produit à plusieurs reprises pour plus de 15 % des utilisateurs de cette stratégie. | Tous les modèles de stratégie | Ajustez l’événement déclencheur pour réduire la fréquence d’entrée des utilisateurs dans l’étendue de la stratégie. |
...
| Utilisation incorrecte des données générales des patients | 5,000 |
