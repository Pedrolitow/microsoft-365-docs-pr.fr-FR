...
|Modèle de stratégie|Événements déclencheurs pour les stratégies|Configuration requise|
|---|---|---|
|**Vol de données par des employés quittant votre organisation**|Indicateur de date de démission ou de résiliation du connecteur RH ou suppression de compte Azure Active Directory|(facultatif) Connecteur Microsoft 365 RH configuré pour les indicateurs de date de démission ou de licenciement|
...
- **Collection** : ces signaux de catégorie se concentrent sur les activités de téléchargement par des utilisateurs de stratégie dans l’étendue. Il peut s’agir, par exemple, de télécharger des fichiers à partir de sites SharePoint ou de déplacer des fichiers dans un dossier compressé.
...
- **Dissimulation** : ces signaux de catégorie se concentrent sur le masquage des activités à risque par des utilisateurs de stratégie dans l’étendue. Il peut s’agir, par exemple, de renommer des fichiers sur un appareil ou de supprimer ou de rétrograder des étiquettes de confidentialité sur SharePoint fichiers.
...
Pour afficher rapidement l’état d’intégrité d’une stratégie, naviguez vers l’onglet Stratégie et la colonne État. Ici, vous verrez les options d’état d’intégrité de stratégie suivantes pour chaque stratégie :
