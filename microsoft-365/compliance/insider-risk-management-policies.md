...
Les utilisateurs à l’extérieur, qu’ils soient positifs ou négatifs, peuvent présenter des risques plus élevés pour les violations de la stratégie de sécurité. Pour vous aider à vous protéger contre les violations de sécurité involontaires ou malveillantes pour les utilisateurs qui se déconnectent, ce modèle de stratégie utilise les alertes Defender pour les points de terminaison afin de fournir des informations sur les activités liées à la sécurité. Ces activités incluent l’utilisateur qui installe des programmes malveillants ou d’autres applications potentiellement dangereuses et désactive les fonctionnalités de sécurité sur leurs appareils. Les indicateurs de stratégie sont activés une fois que les utilisateurs ont une date de retrait ou d’arrêt importée du connecteur RH de Microsoft 365 en tant qu’événement déclencheur.
...
Vous devrez également avoir Microsoft Defender pour point de terminaison configuré dans votre organisation et activer l’intégration de la gestion des risques initiés par Defender pour le point de terminaison dans le centre de sécurité de Defender pour importer les alertes de violation de sécurité. Pour plus d’informations sur la configuration de Defender pour le point de terminaison pour l’intégration de la gestion des risques internes, consultez la rubrique [configure Advanced Features in Defender for Endpoint](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/advanced-features#share-endpoint-alerts-with-microsoft-compliance-center).
...
| **Modèle de stratégie** | **Déclenchement des événements pour les stratégies** | **Configuration requise** |
| :------------------ | :--------------------------------- | :---------------- |
...
| Violations de stratégie de sécurité générale | Fraude défensive concernant les contrôles de sécurité ou les logiciels indésirables détectés par Microsoft Defender pour le point de terminaison | Active Microsoft Defender pour l’abonnement au point de terminaison <br><br> Intégration de Microsoft Defender pour les points de terminaison au centre de conformité Microsoft 365 configuré |
| Violations de stratégie de sécurité en faisant départion des utilisateurs | Démissions ou indicateurs de date de fin du connecteur RH | Connecteur RH Microsoft 365 configuré pour les indicateurs de date de fin et de suppression <br><br> Active Microsoft Defender pour l’abonnement au point de terminaison <br><br> Intégration de Microsoft Defender pour les points de terminaison au centre de conformité Microsoft 365 configuré |
...
| Violations de stratégie de sécurité par un utilisateur mécontent | Amélioration des performances, performances médiocres ou indicateurs de modification au niveau du poste du connecteur RH | Connecteur RH Microsoft 365 configuré pour les indicateurs disgruntlement <br><br> Active Microsoft Defender pour l’abonnement au point de terminaison <br><br> Intégration de Microsoft Defender pour les points de terminaison au centre de conformité Microsoft 365 configuré |
