...
- **Indicateur de violation de stratégie de sécurité**: ils incluent des indicateurs de Microsoft Defender pour le point de terminaison liés à l’installation de logiciels non approuvés ou malveillants ou à contourner les contrôles de sécurité. Pour recevoir des alertes dans la gestion des risques initiés, vous devez disposer d’une intégration active Defender pour les risques internes et Insider. Pour plus d’informations sur la configuration de Defender pour le point de terminaison pour l’intégration de la gestion des risques d’initiés, voir [configure Advanced Features in Microsoft Defender for Endpoint](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/advanced-features\#share-endpoint-alerts-with-microsoft-compliance-center).
...
3. Installez le navigateur Edge de Microsoft chrome sur l’appareil de point de terminaison pour surveiller les actions de l’activité de chargement Cloud. Si vous souhaitez en savoir plus, veuillez consulter la rubrique [Télécharger le nouveau Microsoft Edge basé sur Chromium](https://support.microsoft.com/help/4501095/download-the-new-microsoft-edge-based-on-chromium).
...
Les alertes de Defender pour le point de terminaison sont importées quotidiennement. En fonction de l’état de triage que vous choisissez, vous pouvez voir plusieurs activités utilisateur pour la même alerte que les modifications d’état de triage apportées dans Defender for Endpoint.

Par exemple, si vous sélectionnez *nouveau*, *en cours* et *résolu* pour ce paramètre, lorsqu’une alerte Microsoft Defender pour le point de terminaison est générée et que l’État est *nouveau*, une activité d’alerte initiale est importée pour l’utilisateur en cas d’Insider. Lorsque l’état de triage défenseur pour le point de terminaison devient *en cours*, une deuxième activité pour cette alerte est importée pour l’utilisateur en cas d’Insider. Lorsque l’état de triage de point de terminaison de *finalisation est défini* , une troisième activité pour cette alerte est importée pour l’utilisateur en cas d’Insider. Cette fonctionnalité permet aux enquêteurs de suivre la progression des alertes de défenseur pour les points de terminaison et de choisir le niveau de visibilité requis par leur enquête.
...
| Status | État de l’alerte. Les valeurs sont *actives* (*examen* des risques Insider), *étude* (*confirmée* en ce qui se traduit par des risques Insiders), *résolues* (*résolues* en risque d’initiés), *ignorées* (*rejetée* en risque d’initiés). |
