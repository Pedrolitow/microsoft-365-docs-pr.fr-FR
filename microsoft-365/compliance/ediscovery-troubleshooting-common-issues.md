...
La sortie de’useralias@contoso.com’peut être
...
## <a name="errorissue-search-fails-on-specific-locations"></a>Échec de la recherche d’erreur/problème sur des emplacements spécifiques
...
Si vous recevez cette erreur, nous vous recommandons de vérifier les emplacements ayant échoué dans la recherche, puis de ré-exécuter la recherche uniquement sur les emplacements ayant échoué.

1. Connectez-vous à [Exchange Online Protection PowerShell](https://docs.microsoft.com/en-us/powershell/exchange/exchange-eop/connect-to-exchange-online-protection-powershell?view=exchange-ps).
1. Type :
...
3. À partir de la sortie PowerShell, affichez les emplacements en échec dans le champ erreurs ou à partir des informations d’État dans l’erreur du résultat de la recherche.
1. Renouvelez la recherche de découverte électronique aux emplacements ayant échoué uniquement.
1. Si vous continuez à recevoir ces erreurs, reportez-vous à la rubrique [Retry failed locations](https://docs.microsoft.com/en-us/Office365/SecurityCompliance/retry-failed-content-search) pour connaître les étapes de dépannage supplémentaires.

## <a name="errorissue-file-not-found"></a>Erreur/fichier de problème introuvable

Lors de l’exécution d’une recherche de découverte électronique incluant SharePoint Online et un lecteur pour les emplacements d’entreprise `File Not Found` , vous pouvez recevoir le message d’erreur même si le fichier se trouve sur le site. Cette erreur se produit dans les avertissements et les erreurs d’exportation. csv ou éléments ignorés. csv cela peut se produire si le fichier est introuvable sur le site ou si l’index est obsolète. Voici le texte d’une erreur réelle, avec l’accentuation ajoutée.
...
## <a name="errorissue-search-fails-recipient-not-found"></a>Échec de la recherche d’erreur/problème le destinataire est introuvable

la recherche de découverte électronique `recipient not found`échoue avec une erreur. Cette erreur peut se produire si l’objet utilisateur est introuvable dans Exchange Online Protection (EOP) car l’objet n’a pas été synchronisé.

### <a name="resolution"></a>Résolution

1. Connectez-vous à [Exchange Online Protection PowerShell](https://docs.microsoft.com/en-us/powershell/exchange/exchange-eop/connect-to-exchange-online-protection-powershell?view=exchange-ps).
1. Vérifiez si l’objet utilisateur est synchronisé avec le type de protection Exchange Online :

```powershell
Get-Recipient userId|fl
```

3. Il doit y avoir un objet MailUser pour la question de l’utilisateur. Si aucune valeur n’est renvoyée, examinez l’objet User. Contacter CSS si l’objet ne peut pas être synchronisé.
...
2.  Si vous rencontrez toujours des problèmes, connectez-vous à [Exchange Online Protection PowerShell](https://docs.microsoft.com/en-us/powershell/exchange/exchange-eop/connect-to-exchange-online-protection-powershell?view=exchange-ps) et tapez :
...
2. Connectez-vous à [Exchange Online Protection PowerShell](https://docs.microsoft.com/en-us/powershell/exchange/exchange-eop/connect-to-exchange-online-protection-powershell?view=exchange-ps) et tapez :
...
3. Examinez le fichier trace. log. Il se trouve dans le dossier vers lequel vous avez envoyé l’exportation.
4. Contactez le support technique CSS.

## <a name="errorissue-holds-dont-sync"></a>Erreurs/problèmes suspendus ne pas synchroniser
...
1.  Connectez-vous à [Exchange Online Protection PowerShell](https://docs.microsoft.com/en-us/powershell/exchange/exchange-eop/connect-to-exchange-online-protection-powershell?view=exchange-ps) et tapez :
...
3. Code CSS des contacts.

## <a name="see-also"></a>Voir aussi
- [Conseils pour éviter les erreurs d’emplacement de contenu](https://docs.microsoft.com/en-us/microsoft-365/compliance/retry-failed-content-search%23tips-to-avoid-content-location-errors)