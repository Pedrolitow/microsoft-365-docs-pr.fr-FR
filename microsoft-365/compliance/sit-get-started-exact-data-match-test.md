...
3. Chargez un élément qui contient les données que vous souhaitez détecter. Par exemple, créez un élément qui contient un sous-ensemble des lignes de votre table d’informations sensibles. Si vous avez utilisé la fonctionnalité de correspondance configurable dans votre schéma pour définir des délimiteurs ignorés, assurez-vous que l’élément inclut des exemples avec et sans ces délimiteurs.
