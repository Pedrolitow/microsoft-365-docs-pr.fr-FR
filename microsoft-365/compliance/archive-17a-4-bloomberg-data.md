...
- purview-compliance
- data-connectors
ms.openlocfilehash: 4b78062f8db692aa4197ebe4373e4be7fd570c45
...
![Flux de travail d’archivage pour les données Bloomberg de 17a à 4.](../media/BloombergDataParserConnectorWorkflow.png)

1. Votre organisation utilise 17a-4 pour configurer Bloomberg DataParser.

2. Régulièrement, les éléments Bloomberg sont collectés par le DataParser. DataParser convertit également le contenu d’un message au format de message électronique.

3. Le connecteur Bloomberg DataParser que vous créez dans le portail de conformité Microsoft Purview se connecte à DataParser et transfère les messages vers un emplacement de stockage Azure sécurisé dans le cloud Microsoft.
...
1. Accédez à <https://compliance.microsoft.com> , puis sélectionnez **Connecteurs** >  de données **Bloomberg DataParser**.

2. Dans la page de description du produit **Bloomberg DataParser** , sélectionnez **Ajouter un connecteur**.
...
Utilisez la prise en charge 17a-4 pour configurer le connecteur Bloomberg DataParser.

## <a name="step-3-map-users"></a>Étape 3 : Mapper les utilisateurs

Le connecteur Bloomberg DataParser mappera automatiquement les utilisateurs à leur adresse e-mail Microsoft 365 avant d’importer des données dans Microsoft 365.

## <a name="step-4-monitor-the-bloomberg-dataparser-connector"></a>Étape 4 : Surveiller le connecteur Bloomberg DataParser

Après avoir créé un connecteur Bloomberg DataParser, vous pouvez afficher l’état du connecteur dans le portail de conformité.

1. Accédez à <https://compliance.microsoft.com> et sélectionnez **Connecteurs de données** dans la navigation de gauche.

2. Sélectionnez l’onglet **Connecteurs** , puis sélectionnez le connecteur Bloomberg DataParser que vous avez créé pour afficher la page de menu volant, qui contient les propriétés et les informations sur le connecteur.
