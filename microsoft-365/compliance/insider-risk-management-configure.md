...
Consultez [](import-healthcare-data.md) l’article Configurer un connecteur pour importer des données de santé ou Configurer un connecteur pour importer des données [EHR d’Ehr Pour](import-epic-data.md) obtenir des instructions pas à pas sur la configuration d’un connecteur spécifique à la santé pour votre organisation. Une fois que vous avez configuré un connecteur, revenir à ces étapes de configuration.
