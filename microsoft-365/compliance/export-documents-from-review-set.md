...
- **Options de sortie** : le contenu exporté est disponible en téléchargement directement via un navigateur web ou peut être envoyé à un compte stockage Azure. Les deux premières options permettent le téléchargement direct.
  ...
  - Structure d’annuaire condensée exportée vers votre compte stockage Azure : les fichiers sont exportés vers le compte stockage Azure de votre organisation. Pour cette option, vous devez fournir l’URL du conteneur dans votre compte stockage Azure vers lequel exporter les fichiers. Vous devez également fournir le jeton de signature d’accès partagé (SAP) pour votre compte stockage Azure. Pour plus d’informations, consultez [Exporter des documents dans une révision définie sur un compte stockage Azure](download-export-jobs.md).
...
- Dossier racine : ce dossier nommé [Nom d’exportation] x de z.zip est répété pour chaque partition de fichier ZIP. Le dossier racine contient les éléments suivants :
  ...
  - Exchange : ce dossier contient tout le contenu de Exchange stocké dans des fichiers PST. Les fichiers PDF expurgés ne peuvent pas être inclus avec cette option. Si une pièce jointe est sélectionnée dans le jeu de révision, l’e-mail parent est exporté avec la pièce jointe.
...
### <a name="condensed-directory-structure-exported-to-your-azure-storage-account"></a>Structure de répertoire condensée exportée vers votre compte stockage Azure

Cette option utilise la même structure générale que la *structure de répertoire condensé*, mais le contenu n’est pas compressé et les données sont enregistrées dans votre compte stockage Azure. Cette option est généralement utilisée lors de l’utilisation d’un fournisseur eDiscovery tiers. Pour plus d’informations sur l’utilisation de cette option, consultez [Exporter des documents dans une révision définie sur un compte stockage Azure](download-export-jobs.md).
