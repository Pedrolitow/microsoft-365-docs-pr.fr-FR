...
![Flux de travail d’archivage pour les données Slack comprises entre 17a et 4.](../media/SlackDataParserConnectorWorkflow.png)

1. Votre organisation utilise 17a-4 pour configurer Et configurer Slack DataParser.

2. Régulièrement, les éléments Slack sont collectés par le DataParser. DataParser convertit également le contenu d’un message au format de message électronique.

3. Le connecteur Slack DataParser que vous créez dans le portail de conformité Microsoft Purview se connecte à DataParser et transfère les messages vers un emplacement de stockage Azure sécurisé dans le cloud Microsoft.

4. Un sous-dossier dans le dossier Boîte de réception nommé **Slack DataParser** est créé dans les boîtes aux lettres utilisateur, et les éléments Slack sont importés dans ce dossier. Le connecteur détermine la boîte aux lettres dans laquelle importer les éléments à l’aide de la valeur de la propriété *Email*. Chaque élément Slack contient cette propriété, qui est renseignée avec l’adresse e-mail de chaque participant.
...
1. Accédez à <https://compliance.microsoft.com> , puis sélectionnez **Connecteurs** >  de données **Slack DataParser**.
...
5. Connectez-vous à votre compte 17a-4 et suivez les étapes décrites dans l’Assistant Connexion à Slack DataParser.

## <a name="step-2-configure-the-slack-dataparser-connector"></a>Étape 2 : Configurer le connecteur Slack DataParser

Utilisez la prise en charge 17a-4 pour configurer le connecteur Slack DataParser.

## <a name="step-3-map-users"></a>Étape 3 : Mapper les utilisateurs

Le connecteur Slack DataParser mappera automatiquement les utilisateurs à leurs adresses e-mail Microsoft 365 avant d’importer des données dans Microsoft 365.
