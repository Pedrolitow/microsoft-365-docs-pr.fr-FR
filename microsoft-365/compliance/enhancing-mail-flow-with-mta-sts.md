...
Ces stratégies ne sont pas quelque chose que Exchange Online pouvez héberger pour le compte des clients. Les clients doivent donc configurer la stratégie STS de leur domaine à l’aide des services qu’ils préfèrent. Les services Azure peuvent être facilement utilisés pour l’hébergement de stratégies et une procédure de configuration est disponible plus loin dans cet article. La stratégie doit être protégée par HTTPS avec un certificat pour le sous-domaine `mta-sts.<domain name>`.
...
### <a name="configuring-inbound-mta-sts-with-azure-services"></a>Configuration de MTA-STS entrant avec Azure Services
...
2. Dans **Repos > Files**, clonez votre référentiel dans n’importe quel IDE de votre choix. Dans cet exemple, le dépôt sera cloné dans Visual Studio.
...
3. Une fois le référentiel cloné, créez le chemin d’accès au dossier suivant : `home\.well-known\`. Ensuite, créez les fichiers suivants : 
    ...
    - Fichier 1 :well-known\mta-sts.txt d’accueil\.

   > [!NOTE]
   > Cette configuration permet uniquement à Exchange Online de recevoir des messages pour le compte de votre domaine. Si vous utilisez plusieurs fournisseurs de messagerie, vous devez également référencer des hôtes MX pour les domaines de ces autres fournisseurs. Les caractères génériques ou « * » ne doivent pas être utilisés comme préfixe MX dans tous les scénarios MTA-STS ; Les paramètres ci-dessous sont spécifiques à Exchange Online uniquement et ne doivent PAS être utilisés comme conseils généraux pour la configuration de MTA-STS. 
    ...
    1. Entrez le texte suivant dans le fichier mta-sts.txt :
        ...
        ```
       > [!NOTE]
       > Il est recommandé que le mode de stratégie soit initialement défini en tant que « test ». Ensuite, à la fin de la configuration et après avoir validé que la stratégie fonctionne comme prévu, mettez à jour le fichier mta-sts.txt de sorte que le mode soit « appliqué ».
 
       Le fichier doit contenir uniquement le contenu, comme illustré dans la capture d’écran suivante :
    ...
    1. Créez un fichier index.html et entrez le code suivant dans celui-ci :
      ...
      Une fois le chemin d’accès et les fichiers du dossier créés, n’oubliez pas de valider les modifications et de les envoyer dans votre branche principale.
...
5. Une fois la création de l’application web statique terminée et l’approvisionnement de la ressource, accédez à **Vue d’ensemble > Gérer le jeton de déploiement** ; copiez ensuite le jeton tel qu’il sera utilisé à l’étape suivante.
...
6. Accédez à **Pipelines > Créer un pipeline > Azure Repos Git > STS_Project MTA**, puis effectuez les tâches subordonnées suivantes :
    1. Accédez à **Variables > Nouvelle variable** et tapez ce qui suit :
        1. **Nom** : jeton
        1. **Valeur** : (collez le jeton que vous avez précédemment copié)
    1. Une fois la variable enregistrée, revenez à **Vérifier votre yaml de pipeline** et collez le code yml suivant, enregistrez-le et exécutez-le.
...
8. Ajoutez votre domaine personnel dans **Azure Static Web App > domaines personnalisés > Ajouter**. Vous devrez créer un enregistrement **CNAME** via votre fournisseur DNS (par exemple, GoDaddy) pour vérifier que la zone vous appartient. Une fois la validation terminée, Azure émet un certificat et le lie automatiquement à votre application web statique.

9. Vérifiez que votre stratégie MTA-STS est publiée via : https://mta-sts.[ votre domaine]/.bien connu/mta-sts.txt.
...
10. Créez l’enregistrement DNS TXT MTA-STS via votre fournisseur DNS. Le format est le suivant :
    ...
    > [!NOTE]
    > Vous trouverez un exemple d’enregistrement TXT MTA-STS dans [How To Adopt MTA-STS](#how-to-adopt-mta-sts).

11. Une fois l’enregistrement TXT disponible dans DNS, validez votre configuration MTA-STS. Une fois la configuration validée, mettez à jour le fichier mta-sts.txt afin que le mode de stratégie soit « appliqué » ; mettez ensuite à jour votre ID de stratégie dans l’enregistrement TXT.
...
1. Créez une application de fonction Azure avec la configuration suivante :
    ...
    - **Nom de l’application de fonction** : [Comme votre choix]
    - **Publier** : Code
    - **Pile d’exécution** : .NET
    - **Version** : 6
    - **Système d’exploitation** : Windows
    - **Type de plan** : [Comme votre choix]

    :::image type="content" source="../media/new-azure-function-app.png" alt-text="Capture d’écran montrant les configurations d’une nouvelle application Azure Function." lightbox="../media/new-azure-function-app.png":::
...
3. Liez vos mta-sts. [votre domaine] à l’application de fonction.
...
4. Dans **Le fichier d’application**, ajoutez l’extension suivante au fichier host.json de votre application de fonction pour éliminer le routePrefix. Cet ajout est nécessaire pour supprimer la /api de l’URL de la fonction.
...
5. Dans votre application de fonction, accédez à **Functions > Créer** et configurer les paramètres suivants : 

   > [!NOTE]
   > Bien que cet exemple décrive le développement de fonctions via le portail, vous êtes libre d’utiliser VS Code ou tout autre outil que vous préférez.
    ...
    - **Nouvelle fonction** : [Comme votre choix]
...
6. Une fois la fonction créée, ouvrez **Code + Test** et développez en C# une réponse HTTP asynchrone simple qui sera votre stratégie MTA-STS. L’exemple suivant indique que Exchange Online doit recevoir des e-mails :
...
7. Dans **Integration > HTTP (req)**, modifiez le déclencheur en fonction des valeurs suivantes :

    - **Modèle de routage** : .well-known/mta-sts.txt
...
8. Vérifiez que votre stratégie MTA-STS est publiée via : https://mta-sts.[ votre domaine]/.bien connu/mta-sts.txt.
...
10. Une fois l’enregistrement TXT disponible dans DNS, validez votre configuration MTA-STS. Une fois la configuration validée, mettez à jour le fichier mta-sts.txt de sorte que le mode de stratégie soit « appliqué » ; mettez ensuite à jour votre ID de stratégie dans l’enregistrement TXT.