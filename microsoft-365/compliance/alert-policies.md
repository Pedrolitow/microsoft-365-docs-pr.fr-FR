...
|**Programme malveillant non zapped car ZAP est désactivé**| Génère une alerte lorsque Microsoft détecte la remise d’un message malveillant à une boîte aux lettres car Zero-Hour la purge automatique des messages hameçons est désactivée. Cette stratégie a un paramètre de gravité d' **information** . |Gestion des menaces|L’abonnement de compléments E5/G5 ou Defender pour Office 365 P2|
|**Hameçonnage remis car le dossier de courrier indésirable d’un utilisateur est désactivé**|Génère une alerte lorsque Microsoft détecte que le dossier de courrier indésirable d’un utilisateur est désactivé, ce qui permet la remise d’un message de hameçonnage à une boîte aux lettres. Cette stratégie a un paramètre de gravité d' **information** .|Gestion des menaces|E5/G5 ou Defender pour l’abonnement au complément Office 365 P1 ou P2|
|**Hameçonnage envoyé en raison d’un remplacement ETR**|Génère une alerte lorsque Microsoft détecte une règle de transport Exchange (ETR) qui a autorisé la remise d’un message de hameçonnage à une boîte aux lettres. Cette stratégie a un paramètre de gravité d' **information** . Pour plus d’informations sur les règles de transport Exchange (règles de flux de messagerie), consultez la rubrique [mail Flow Rules (règles de transport) dans Exchange Online](https://docs.microsoft.com/exchange/security-and-compliance/mail-flow-rules/mail-flow-rules).|Gestion des menaces|E5/G5 ou Defender pour l’abonnement au complément Office 365 P1 ou P2|
|**Hameçonnage envoyé en raison d’une stratégie d’autorisation IP**|Génère une alerte lorsque Microsoft détecte une stratégie d’autorisation IP qui a autorisé la remise d’un message de hameçonnage à une boîte aux lettres. Cette stratégie a un paramètre de gravité d' **information** . Pour plus d’informations sur la stratégie d’autorisation IP (filtrage des connexions), voir [configure the default connection filter Policy-Office 365](https://docs.microsoft.com/microsoft-365/security/office-365-security/configure-the-connection-filter-policy).|Gestion des menaces|E5/G5 ou Defender pour l’abonnement au complément Office 365 P1 ou P2|
|**Hameçonnage non zapped car ZAP est désactivé**| Génère une alerte lorsque Microsoft détecte la remise d’un message de hameçonnage à une boîte aux lettres car Zero-Hour la purge automatique des messages hameçons est désactivée. Cette stratégie a un paramètre de gravité d' **information** .|Gestion des menaces|L’abonnement de compléments E5/G5 ou Defender pour Office 365 P2|
...
Pour voir la catégorie affectée à une stratégie d’alerte par défaut, consultez le tableau dans [stratégies d’alerte par défaut](#default-alert-policies).
